...
function Settings({settings,setSettings,businessDay,toggleBusiness,taxRate,setTaxRate,company,onCancel}){
 ...
 const testDisplay=()=>{const mode=cd.secondaryMonitor?"secondary":String(hw.customerDisplayMode||"COM").toLowerCase();const payload={port:cd.comPort||hw.customerDisplayPort,baud:Number(cd.baudRate||hw.customerDisplayBaud||9600),dataBits:Number(cd.dataBits||8),stopBits:Number(cd.stopBits||1),line1:cd.topLine||hw.customerDisplayTop||"WELCOME!",line2:cd.bottomLine||hw.customerDisplayBottom||"",chars:Number(cd.characters||20),mode};hardwareRequestLocal(hw.agentUrl,"/display",payload).then(()=>mode==="secondary"?hardwareRequestLocal(hw.agentUrl,"/display-window",{}):null).then(result=>setNotice(mode==="secondary"?(result?.details?.windowFound===false?"Customer display opened on monitor 2, but the browser window handle was not detected. Check the second monitor.":"Customer display opened on the secondary monitor successfully."):"Customer display test sent successfully.")).catch(e=>setNotice("Customer display test failed: "+e.message));};
