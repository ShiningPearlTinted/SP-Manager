...
function App(){
 ...
 return <div className={"app app-theme-"+String(settings.general.colorScheme||"Light").toLowerCase().replace(/\s+/g,"-")} dir={settings.general.direction||"ltr"} style={{zoom:Number(settings.general.zoom||100)/100}}>
  ...
  {page==="Loyalty"&&<Loyalty customers={customers} setCustomers={setCustomers}/>}
   ...
   {page==="Settings"&&isPermissionAllowed(activeUser,"manageSettings")&&<Settings settings={settings} setSettings={updateSettings} businessDay={businessDay} toggleBusiness={toggleBusiness} taxRate={taxRate} setTaxRate={r=>{setTaxRate(r);save("taxRate",r)}} company={company} customerDisplayTerminal={customerDisplayTerminal} setCustomerDisplayTerminal={next=>{setCustomerDisplayTerminal(prev=>{const resolved=typeof next==="function"?next(prev):next;save("customerDisplayTerminal",resolved);return resolved})}} onCancel={()=>setPage("POS / Sales")}/>}
