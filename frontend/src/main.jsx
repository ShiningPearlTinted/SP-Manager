...
const normalizeProductStockControl=p=>{
 ...
 return {...p,reorder,preferredQuantity:Number.isFinite(Number(p?.preferredQuantity))?Math.max(0,Number(p.preferredQuantity)):reorder,lowStockWarning:p?.lowStockWarning!==undefined?Boolean(p.lowStockWarning):reorder>0,lowStockWarningQuantity:warningQty,supplierId:p?.supplierId||""};
};
...
const invoiceAddress=company=>[company?.streetName,company?.buildingNumber,company?.additionalStreetName,company?.plotIdentification,company?.district,company?.postalCode,company?.city,company?.state,company?.country].filter(Boolean).join(", ");
const buildInvoiceHtml=(sale,company={},customers=[],settings={})=>{
 ...
 const addr=invoiceAddress(company);
 const dueDate=sale?.dueDate||sale?.date;
 const paid=Number(sale?.paymentAmount||0);
 const total=Number(sale?.total||0);
 const due=Math.max(0,total-paid);
 const paymentLines=(Array.isArray(sale?.payments)&&sale.payments.length?sale.payments:[{payment:sale?.payment||"Cash",amount:paid}]).map(x=>`<div>${escapeHtml(x.payment||"Payment")}: <b>${escapeHtml(money(x.amount))}</b></div>`).join("");
 const itemRows=items.map((i,n)=>`<tr><td>${n+1}</td><td>${escapeHtml(i.name||"")}</td><td class="num">${escapeHtml(i.qty||0)}</td><td class="num">${escapeHtml(Number(i.price||0).toFixed(2))}</td><td class="num">${escapeHtml(money(Number(i.price||0)*Number(i.qty||0)))}</td></tr>`).join("");
 return `<div class="invoice-page">
  <div class="invoice-head"><div class="invoice-company">${logo}<div><div class="company-name">${escapeHtml(company?.name||"Shining Pearl Tinted")}</div>${addr?`<div>${escapeHtml(addr)}</div>`:""}${company?.phoneNumber?`<div>Phone: ${escapeHtml(company.phoneNumber)}</div>`:""}${company?.email?`<div>Email: ${escapeHtml(company.email)}</div>`:""}</div></div><div class="invoice-title">${escapeHtml(settings?.print?.invoiceTitle||"INVOICE")}</div></div>
  <div class="invoice-info-grid"><div><b>Bill to</b><div>${escapeHtml(customer?.name||"Walk-in customer")}</div>${settings?.print?.customerPhone&&customer?.phone?`<div>${escapeHtml(customer.phone)}</div>`:""}${settings?.print?.customerEmail&&customer?.email&&customer.email!=="-"?`<div>${escapeHtml(customer.email)}</div>`:""}</div><div><div><b>Invoice No.:</b> ${escapeHtml(sale?.no||"")}</div><div><b>Date:</b> ${escapeHtml(formatInvoiceDate(sale?.date))}</div><div><b>Due date:</b> ${escapeHtml(formatInvoiceDate(dueDate))}</div><div><b>Payment status:</b> ${sale?.paid===false||due>0?"Unpaid":"Paid"}</div></div></div>
  ${(company?.bankAccountNumber||company?.bankDetails)?`<div class="bank-box">${company?.bankAccountNumber?`<div><b>Bank acc. number:</b> ${escapeHtml(company.bankAccountNumber)}</div>`:""}${company?.bankDetails?`<div><b>Bank details:</b> ${escapeHtml(company.bankDetails)}</div>`:""}</div>`:""}
  <table class="invoice-table"><thead><tr><th>#</th><th>Item</th><th>Quantity</th><th>Unit price</th><th>Tax</th><th>Discount</th><th>Total</th></tr></thead><tbody>${items.map((i,n)=>`<tr><td>${n+1}</td><td>${escapeHtml(i.name||"")}</td><td class="num">${escapeHtml(i.qty||0)}</td><td class="num">${escapeHtml(Number(i.price||0).toFixed(2))}</td><td class="num">---</td><td class="num">0.00%</td><td class="num">${escapeHtml(money(Number(i.price||0)*Number(i.qty||0)))}</td></tr>`).join("")}</tbody></table>
  <div class="invoice-total">Total <b>${escapeHtml(money(total))}</b></div>
  <div class="payment-summary"><div><b>Payment method:</b></div>${paymentLines}<div><b>Paid amount:</b> ${escapeHtml(money(paid))}</div><div><b>Amount due:</b> ${escapeHtml(money(due))}</div></div>
  <div class="invoice-footer">Page 1</div>
 </div>`;
};
const invoiceCss=`@page{size:A4;margin:0}body{font-family:Arial,Segoe UI,sans-serif;color:#111;background:#fff;margin:0}.invoice-page{box-sizing:border-box;width:210mm;min-height:297mm;padding:18mm 16mm 14mm;margin:auto;font-size:11px}.invoice-head{display:flex;justify-content:space-between;align-items:flex-start;border-bottom:1.2px solid #222;padding-bottom:9mm}.invoice-company{display:flex;gap:10px;line-height:1.45}.company-logo{width:42mm;max-height:22mm;object-fit:contain;object-position:left center}.company-name{font-size:15px;font-weight:700;margin-bottom:2px}.invoice-title{font-size:25px;font-weight:700;letter-spacing:.8px}.invoice-info-grid{display:grid;grid-template-columns:1fr 1fr;gap:20mm;margin-top:8mm;line-height:1.65}.bank-box{margin-top:5mm;line-height:1.6}.invoice-table{width:100%;border-collapse:collapse;margin-top:7mm;font-size:10px}.invoice-table th{background:#f4f4f4;border-top:1px solid #222;border-bottom:1px solid #222;padding:7px 5px;text-align:left}.invoice-table td{border-bottom:1px solid #d5d5d5;padding:7px 5px;vertical-align:top}.invoice-table th:nth-child(1),.invoice-table td:nth-child(1){width:7%}.invoice-table th:nth-child(3),.invoice-table td:nth-child(3){width:12%}.invoice-table th:nth-child(4),.invoice-table td:nth-child(4){width:14%}.invoice-table th:nth-child(5),.invoice-table td:nth-child(5){width:12%}.invoice-table th:nth-child(6),.invoice-table td:nth-child(6){width:14%}.invoice-table th:nth-child(7),.invoice-table td:nth-child(7){width:16%}.num{text-align:right}.invoice-total{margin-top:7mm;margin-left:auto;width:62mm;display:flex;justify-content:space-between;font-size:13px;border-top:1.5px solid #222;padding-top:4mm}.invoice-total b{font-size:15px}.payment-summary{margin-top:9mm;margin-left:auto;width:75mm;line-height:1.8}.payment-summary>div{display:flex;justify-content:space-between;gap:8px}.payment-summary>div:first-child{display:block;margin-bottom:2mm}.invoice-footer{margin-top:18mm;text-align:right;font-size:9px;color:#666}@media print{.invoice-page{margin:0}}`;
const printInvoice=(sale,company={},customers=[],settings={})=>{try{const w=window.open("","_blank","width=900,height=760");if(!w){window.alert("Please allow pop-ups for SP-Manager printing.");return false}w.document.open();w.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>Invoice ${escapeHtml(sale?.no||"")}</title><style>${invoiceCss}</style></head><body>${buildInvoiceHtml(sale,company,customers,settings)}<script>window.onload=()=>setTimeout(()=>window.print(),180);<\/script></body></html>`);w.document.close();return true}catch(e){window.alert("Unable to open invoice: "+e.message);return false}};
...
function App(){
 ...
 const emailReceipt=async(sale)=>{
  ...
  try{
   ...
   const r=await fetch(base+"/email",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({host:e.host,port:Number(e.port),ssl:e.ssl!==false,from:e.emailAddress,displayName:e.displayName||company?.name||"SP-Manager",username:e.username||e.emailAddress,password:e.password,to,subject,body,bcc:e.bcc||""})});
 ...
 const disc=Math.min(subtotal,Number(discountFixed||0)+subtotal*Number(discount||0)/100);
 ...
 useEffect(()=>{if(!settings.database.backupOnClose)return;const run=()=>{try{const snapshot={createdAt:new Date().toISOString(),data:{}};for(let i=0;i<localStorage.length;i++){const k=localStorage.key(i);if(k&&k.startsWith("sp_"))snapshot.data[k]=localStorage.getItem(k)}localStorage.setItem("sp_auto_backup_close",JSON.stringify(snapshot));localStorage.setItem("sp_last_backup_close",snapshot.createdAt)}catch{}};window.addEventListener("beforeunload",run);return()=>window.removeEventListener("beforeunload",run)},[settings.database.backupOnClose]);
 useEffect(()=>{if(!settings.database.autoBackup)return;const every=Math.max(1,Number(settings.database.backupEveryHours||24))*3600000;const run=()=>{const snapshot={createdAt:new Date().toISOString(),data:{}};for(let i=0;i<localStorage.length;i++){const k=localStorage.key(i);if(k&&k.startsWith("sp_"))snapshot.data[k]=localStorage.getItem(k)}localStorage.setItem("sp_auto_backup",JSON.stringify(snapshot));setSettings(x=>({...x,database:{...x.database,lastBackup:snapshot.createdAt}}))};if(settings.database.backupOnStart)run();const timer=setInterval(run,every);return()=>clearInterval(timer)},[settings.database.autoBackup,settings.database.backupEveryHours,settings.database.backupOnStart]);
 ...
 const add=(p,quantityOverride)=>{if(!settings.products.allowNegativePrice&&Number(p.price||0)<0){setNotice("Negative price is not allowed for "+p.name+".");return}if(settings.order.preventSaleBelowCost&&!p.service&&!p.serviceItem&&Number(p.price||0)<Number(p.cost||0)){setNotice("Sale below cost price is prevented for "+p.name+".");return}const existing=settings.order.separateRow?null:cart.find(x=>x.id===p.id);const step=Math.max(1,Number(quantityOverride||settings.order.defaultQuantity||1));const nextQty=(existing?.qty||0)+step;if(settings.order.preventNegativeInventory&&!p.service&&!p.serviceItem&&nextQty>Number(p.stock||0)){setNotice("Insufficient stock for "+p.name+".");return}setCart(c=>{const old=settings.order.separateRow?null:c.find(x=>x.id===p.id);const qty=(old?.qty||0)+step;const price=promotionPrice(p,qty);const line=old?{...old,qty,price}:{...p,id:p.id,productId:p.id,lineId:settings.order.separateRow?uid():p.id,qty,originalPrice:p.price,price};return old?c.map(x=>x.id===p.id?line:x):[...c,line]});};
 const changeQty=(key,d)=>{setCart(c=>c.flatMap(x=>{if((x.lineId||x.id)!==key)return [x];const qty=x.qty+d;if(qty<=0)return [];const base=products.find(p=>p.id===x.productId||p.id===x.id)||x;return [{...x,qty,price:promotionPrice(base,qty),originalPrice:base.price}]}));};
 ...
 const completeSale=(paymentInfo={})=>{
  ...
  if(settings.order.showReceiptDialog===false){setLastSale(null);printReceipt(sale)}else setLastSale(sale);
 ...
 const saveOpenOrder=({newSale=false}={})=>{
  ...
  const next=[...orders,order];persist("orders",next,setOrders);setCart([]);setDiscount(0);setDiscountFixed(0);setNotice("Sale saved successfully: "+order.name);setPage("POS / Sales");
 };
 ...
 const printReceipt=(sale)=>{
  const c=customers.find(x=>x.id===sale.customerId);const receiptItems=settings?.order?.mergeItemsOnReceipt===false?(sale.items||[]):Object.values((sale.items||[]).reduce((m,i)=>{const k=String(i.id)+"|"+Number(i.price||0).toFixed(6);const old=m[k];m[k]=old?{...old,qty:Number(old.qty||0)+Number(i.qty||0)}:{...i};return m},{}));const rows=receiptItems.map(i=>`<tr><td>${Number(i.qty||0)}</td><td>${String(i.name||"")}</td><td style="text-align:right">RM${(Number(i.price||0)*Number(i.qty||0)).toFixed(2)}</td></tr>`).join("");
  const receiptText=[
   ...
   "Customer: "+(c?.name||"Walk-in Customer"),
   "--------------------------------",
   ...receiptItems.map(i=>`${String(i.qty||0).padStart(3)} ${String(i.name||"").slice(0,24).padEnd(24)} RM${(Number(i.price||0)*Number(i.qty||0)).toFixed(2)}`),
   "--------------------------------",
   `Total: RM${Number(sale.total||0).toFixed(2)}`,
   `Payment: ${sale.payment||"Cash"}`,
   `Paid: RM${Number(sale.paymentAmount||0).toFixed(2)}`,
   `Change: RM${Number(sale.change||0).toFixed(2)}`,
   settings?.print?.footer||"Thank you", ...(Array.from({length:Math.max(0,Number(settings?.print?.feedLines||0))},()=>""))
  ].filter(Boolean).join("\n");
  const printer=settings?.hardware?.printer||settings?.print?.printer||"";
  if(settings?.print?.printReceipt!==false&&printer){
   hardwareRequest("/print",{printer,text:receiptText,copies:Number(settings?.print?.copies||1)}).then(()=>{if(settings?.hardware?.cashDrawerEnabled&&String(sale.payment||"").toLowerCase().includes("cash"))cashDrawer();setNotice("Receipt printed successfully.")}).catch(()=>{const w=window.open("","_blank","width=420,height=720");if(!w){alert("Please allow pop-ups to print the receipt.");return}w.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>Receipt ${sale.no}</title><style>body{font-family:Arial,sans-serif;font-size:12px;padding:18px;color:#111}pre{white-space:pre-wrap}</style></head><body><pre>${receiptText.replace(/&/g,"&amp;").replace(/</g,"&lt;")}</pre><script>window.onload=()=>setTimeout(()=>window.print(),200)</script></body></html>`);w.document.close();});
   return;
  }
 ...
 return <div className={"app app-theme-"+String(settings.general.colorScheme||"Light").toLowerCase().replace(/\s+/g,"-")} dir={settings.general.direction||"ltr"} style={{zoom:Number(settings.general.zoom||100)/100}}>
  ...
  {page==="Loyalty"&&<Loyalty customers={customers} setCustomers={setCustomers}/>}
   ...
   {noteBox&&<div className="sp-messagebox-backdrop" role="dialog" aria-modal="true" aria-labelledby="sp-notebox-title"><div className="sp-messagebox sp-notebox"><div className="sp-messagebox-icon note" aria-hidden="true">✎</div><div className="sp-messagebox-content"><div className="sp-messagebox-kicker">SP-MANAGER</div><h2 id="sp-notebox-title">Add notes</h2><p className="sp-notebox-help">Add a note to this transaction.</p><textarea autoFocus className="sp-notebox-input" value={noteBox.note||""} onChange={e=>setNoteBox({...noteBox,note:e.target.value})} onKeyDown={e=>{if((e.ctrlKey||e.metaKey)&&e.key==="Enter"){e.preventDefault();updateSaleNote(noteBox.sale,noteBox.note||"");setNoteBox(null)}}} placeholder="Enter notes..." /><button type="button" className="sp-messagebox-close" aria-label="Close" onClick={()=>setNoteBox(null)}>×</button><div className="sp-messagebox-actions sp-notebox-actions"><button type="button" className="sp-notebox-cancel" onClick={()=>setNoteBox(null)}>Cancel</button><button type="button" className="sp-messagebox-ok" onClick={()=>{updateSaleNote(noteBox.sale,noteBox.note||"");setNoteBox(null)}}>OK</button></div></div></div></div>}
  </main>
 </div>
}
...
function NamedOrders({orders,setOrders,customers}){
...
function POS({filtered,q,setQ,posSearchMode,setPosSearchMode,add,cart,changeQty,customers,setCustomers,customer,setCustomer,discount,setDiscount,discountFixed,setDiscountFixed,payment,setPayment,paymentTypes,subtotal,disc,taxRate,setTaxRate,tax,grand,sale,saveOpenOrder,orders,setOrders,updateSaleNote,setNoteBox,clearCurrentSale,printReceipt,closeLastSale,categories,posCategory,setPosCategory,products,company,lastSale,menuOpen,setMenuOpen,setPage,sales,emailReceipt,settings,activeUser,signOut,openCashInOut,openCashDrawer}){
 ...
 const addPos=p=>{if(saleLocked){setNoticeLocal("Sale is locked. Unlock the sale before changing items.");return}add(p,nextQuantity);setNextQuantity(1);setSelectedLineId(p?.id??null);if(settings.order.sounds&&settings.order.soundItemAdded)playPosBeep("ok")};
 ...
 useEffect(()=>{const onKey=e=>{
   ...
   if(e.key==="Escape"){setPaymentScreen(false);setSplitScreen(false);setDiscountScreen(false);setTransferScreen(false);setSavedScreen(false);setQuantityScreen(false);setSelectedLineId(null);document.querySelector('.ar-search-input')?.focus()}
 ...
 return <section className={"ar-pos-shell "+(posFullscreen?"pos-browser-fullscreen":"")}>
  ...
  <div className="ar-main">
   <div className="ar-order-panel">
    ...
    <div className="ar-order-items">{cart.map(i=><div className={"ar-order-item "+((i.lineId||i.id)===selectedLineId?"selected":"")} key={i.lineId||i.id} onClick={()=>setSelectedLineId(i.lineId||i.id)}><button className="ar-item-plus" onClick={(e)=>{e.stopPropagation();addPos(i)}}>＋</button><div><b>{i.name}</b><small>{i.code||'—'} · {money(i.price)} × {i.qty}</small></div><strong>{money(i.price*i.qty)}</strong></div>)}{!cart.length&&<div className="ar-no-items"><span>🛒</span><b>No items</b><small>Select a category or search for a product</small></div>}</div>
   ...
   <div className={"ar-product-panel "+(virtualKeyboard?"keyboard-open":"")}>
    ...
    {settings.general.layout==="Standard" ? <div className="ar-standard-list">{filtered.length?<div className="ar-standard-table">{filtered.map(p=><button key={p.id} onClick={()=>addPos(p)}><span><b>{p.name}</b><small>{p.code||"No code"}{p.barcode?" · "+p.barcode:""}</small></span><strong>{money(p.price)}</strong></button>)}</div>:<Empty text="No products found."/>}</div> : <>
    {q?<><div className="ar-breadcrumb"><span>Products</span><b>›</b><strong>Search results</strong></div><div className="ar-grid-wrap" style={{gridTemplateColumns:`repeat(${Math.max(1,Number(settings.general.columns||5))},minmax(0,1fr))`}}>{filtered.length?filtered.map(p=><button className="ar-product-tile" key={p.id} onClick={()=>{addPos(p)}}><div className="ar-product-image"><img src={productImageSrc(p)} alt=""/></div><div className="product-card-body"><strong>{p.name}</strong><small>{p.code||'No code'}{p.barcode?" · "+p.barcode:""}</small><b>{money(p.price)}</b></div></button>):<Empty text="No products found."/>}</div></>:null}
    ...
    {catLevel==='group'&&<><div className="ar-breadcrumb"><button className="ar-back-link" onClick={()=>{setCatLevel('root');setGroup('');setPosCategory('All Categories')}}>← Start</button><b>›</b><strong>{posCategory}</strong></div><div className="ar-grid-wrap" style={{gridTemplateColumns:`repeat(${Math.max(1,Number(settings.general.columns||5))},minmax(0,1fr))`}}><button className="ar-back-tile ar-back-primary" onClick={()=>{setCatLevel('root');setGroup('');setPosCategory('All Categories')}}><span>←</span><small>Back to Start</small></button>{groups.length?groups.map(groupTile):<Empty text="No groups found in this category."/>}</div></>}
    {catLevel==='items'&&<><div className="ar-breadcrumb"><button onClick={()=>{setCatLevel('root');setGroup('');setPosCategory('All Categories')}}>Products</button><b>›</b><button onClick={()=>setCatLevel('group')}>{posCategory}</button><b>›</b><strong>{group}</strong></div><div className="ar-grid-wrap" style={{gridTemplateColumns:`repeat(${Math.max(1,Number(settings.general.columns||5))},minmax(0,1fr))`}}><button className="ar-back-tile" onClick={()=>setCatLevel('group')}><span>←</span><small>Back to {posCategory}</small></button>{shown.map(p=><button className="ar-product-tile" key={p.id} onClick={()=>{addPos(p)}}><div className="ar-product-image"><img src={productImageSrc(p)} alt=""/></div><div className="product-card-body"><strong>{p.name}</strong><small>{p.code||'No code'}</small><b>{money(p.price)}</b></div></button>)}</div></>}
    </>}
...
function Products({products,setProducts,addProduct,updateProduct,editing,setEditing,categories,setCategories,productGroups,setProductGroups,suppliers=[],setNotice,settings=defaultSettings}){
 ...
 const categoryGroups=selectedCategory==="All Products"?groups:groups.filter(g=>groupCategories[g]===selectedCategory||products.some(p=>(p.category||"")===selectedCategory&&(p.group||p.category)===g));
 const editorGroups=form.category?groups.filter(g=>groupCategories[g]===form.category||products.some(p=>(p.category||"")===form.category&&(p.group||p.category)===g)||g===form.group):groups;
 ...
 const openNew=()=>{setEditing(null);setForm({...blank,code:nextProductCode(),category:selectedCategory!=="All Products"?selectedCategory:"",group:selectedGroup!=="All Groups"?selectedGroup:""});setShowEditor(true)};
 ...
 const editGroup=()=>{if(selectedGroup==="All Groups"){setNotice("Please select a product group first.");return}setEditGroupName(selectedGroup);setShowEditGroup(true);};
 const saveEditGroup=()=>{const name=editGroupName.trim();if(!name||name===selectedGroup)return;setGroupConfirm({type:"edit",oldName:selectedGroup,name});};
 const deleteGroup=()=>{if(selectedGroup==="All Groups"){setNotice("Please select a product group first.");return}const count=products.filter(p=>(p.group||p.category)===selectedGroup).length;if(count){setNotice("This group contains "+count+" product(s). Move or remove the products before deleting the group.");return}setGroupConfirm({type:"delete",name:selectedGroup});};
 ...
 const openGroupModal=()=>{setNewGroup("");setGroupError("");setShowGroup(true)};
 const saveGroup=e=>{e?.preventDefault();const name=newGroup.trim();if(!name){setGroupError("Enter a product group name.");return}if(groups.some(g=>g.toLowerCase()===name.toLowerCase())){setGroupError("This product group already exists.");return}const parent=selectedCategory!=="All Products"?selectedCategory:"";setGroupConfirm({type:"save",name,parent});};
 const confirmGroupAction=()=>{if(!groupConfirm)return;const {type,name,parent,oldName}=groupConfirm;if(type==="save"){const next=[...groups,name];const categoryMap={...groupCategories,...(parent?{[name]:parent}:{})};save("productGroups",next);save("productGroupCategories",categoryMap);setProductGroups(next);setGroupCategories(categoryMap);setNewGroup("");setGroupError("");setShowGroup(false);setForm(f=>({...f,group:name,category:parent||f.category}));if(parent)setExpandedCategories(a=>new Set(a).add(parent));setNotice("Product group added successfully.")}if(type==="edit"){const next=groups.map(g=>g===oldName?name:g);const np=products.map(p=>(p.group||p.category)===oldName?{...p,group:name,updatedAt:new Date().toISOString()}:p);save("productGroups",next);setProductGroups(next);save("products",np);setSelectedGroup(name);setShowEditGroup(false);setNotice("Product group updated successfully.")}if(type==="delete"){const next=groups.filter(g=>g!==name);save("productGroups",next);setProductGroups(next);setSelectedGroup("All Groups");setNotice("Product group deleted successfully.")}setGroupConfirm(null)};
 ...
 return <section className="content product-master-modern">
  <div className="toolbar product-master-toolbar"><div><h3>Products</h3><small>Product master, product groups, barcodes, pricing and stock control</small></div><div className="product-action-strip"><button title="Refresh" onClick={()=>{setFilter("");setNotice("Product list refreshed successfully.")}}>↻ <span>Refresh</span></button><button title="New group" onClick={(e)=>{e.stopPropagation();openGroupModal()}}>＋ <span>New group</span></button><button title="Edit group" onClick={editGroup}>✎ <span>Edit group</span></button><button title="Delete group" onClick={deleteGroup}>▢ <span>Delete group</span></button><button title="New product" onClick={openNew}>＋ <span>New product</span></button><button title="Edit product" onClick={editSelectedProduct}>✎ <span>Edit product</span></button><button title="Print" onClick={()=>window.print()}>▣ <span>Print</span></button><button title="Save as PDF" onClick={()=>downloadReportPDF("Products",["Code","Name","Group","Barcode","Cost","Sale Price"],visible.map(p=>[p.code||"-",p.name,p.group||p.category||"-",p.barcode||"-",money(p.cost),money(p.price)]))}>PDF <span>Save as PDF</span></button><button title="Price tags" onClick={()=>setShowPriceTags(true)}>▤ <span>Price tags</span></button><button title="Moving average price" onClick={movingAverage}>↯ <span>Mov. avg. price</span></button><button title="Import" onClick={importProducts}>↓ <span>Import</span></button><button title="Export to Excel" onClick={exportProducts}>↑ <span>Export to Excel</span></button><button title="Help" onClick={()=>setNotice("Products supports categories, groups, multiple products per group, multiple barcodes, pricing, stock, tax settings and product details." )}>? <span>Help</span></button></div></div>
  <div className="product-master-shell">
  <aside className="product-tree"><div className="tree-title">Product Groups</div><button className={selectedCategory==="All Products"?"tree-item selected":"tree-item"} onClick={()=>{setSelectedCategory("All Products");setSelectedGroup("All Groups")}}>▣ <span>All Products</span><em>{products.length}</em></button>{unassignedGroups.length>0&&<div className="tree-category"><button className="tree-item" onClick={()=>setExpandedCategories(a=>{const n=new Set(a);n.has("__other")?n.delete("__other"):n.add("__other");return n})}>{expandedCategories.has("__other")?"▾":"▸"} <span>Other groups</span><em>{unassignedGroups.length}</em></button>{expandedCategories.has("__other")&&unassignedGroups.map(g=><button key={g} className={selectedGroup===g?"tree-item child selected":"tree-item child"} onClick={()=>{setSelectedCategory("All Products");setSelectedGroup(g)}}>▪ <span>{g}</span><em>{products.filter(p=>(p.group||p.category)===g).length}</em></button>)}</div>}{categories.map(c=>{const cg=groups.filter(g=>g!==c&&(groupCategories[g]===c||products.some(p=>(p.category||"")===c&&(p.group||p.category)===g)));const expanded=expandedCategories.has(c);return <div key={c} className="tree-category"><button className={selectedCategory===c&&selectedGroup==="All Groups"?"tree-item selected":"tree-item"} onClick={()=>{setSelectedCategory(c);setSelectedGroup("All Groups");setExpandedCategories(a=>{const n=new Set(a);n.has(c)?n.delete(c):n.add(c);return n})}}>{expanded?"▾":"▸"} <span>{c}</span><em>{products.filter(p=>(p.category||p.group)===c).length}</em></button>{expanded&&cg.map(g=><button key={g} className={selectedGroup===g?"tree-item child selected":"tree-item child"} onClick={()=>{setSelectedCategory(c);setSelectedGroup(g)}}>▪ <span>{g}</span><em>{products.filter(p=>(p.group||p.category)===g).length}</em></button>)}</div>})}</aside>
  ...
  {showEditGroup&&<div className="modal-backdrop" onMouseDown={()=>setShowEditGroup(false)}><form className="modal edit-group-modal" onSubmit={e=>{e.preventDefault();saveEditGroup()}} onMouseDown={e=>e.stopPropagation()}><div className="modal-head"><div><span className="eyebrow">PRODUCT GROUPS</span><h3>Edit Group</h3></div><button type="button" className="iconbtn" aria-label="Close" onClick={()=>setShowEditGroup(false)}>×</button></div><label className="edit-group-field">Group name<input autoFocus value={editGroupName} onChange={e=>setEditGroupName(e.target.value)}/></label><div className="edit-group-actions"><button type="button" className="secondary" onClick={()=>setShowEditGroup(false)}>Cancel</button><button type="submit">Save</button></div></form></div>}
 </section>
}
...
function Settings({settings,setSettings,businessDay,toggleBusiness,taxRate,setTaxRate,company,onCancel}){
 ...
 const hardwareRequestLocal=async(base,path,body)=>{const r=await fetch(String(base||"http://127.0.0.1:18765").replace(/\/$/,"")+path,{method:body?"POST":"GET",headers:body?{"Content-Type":"application/json"}:undefined,body:body?JSON.stringify(body):undefined});if(!r.ok)throw Error("HTTP "+r.status);return r.json()};
 const save=()=>{setSettings(draft);setNotice("Settings saved successfully.")};
 const testPrinter=()=>hardwareRequestLocal(hw.agentUrl,"/print",{printer:hw.printer||pr.printer,text:(pr.header?pr.header+"\n":"")+"SP-Manager\nPrinter test\n"+(pr.footer||"")+"\n",copies:Number(pr.copies||1)}).then(()=>setNotice("Printer test sent successfully.")).catch(e=>setNotice("Printer test failed: "+e.message));
 ...
 const testDisplay=()=>hardwareRequestLocal(hw.agentUrl,"/display",{port:cd.comPort||hw.customerDisplayPort,baud:Number(cd.baudRate||hw.customerDisplayBaud||9600),line1:cd.topLine||"WELCOME!",line2:cd.bottomLine||"",chars:Number(cd.characters||20)}).then(()=>setNotice("Customer display test sent successfully.")).catch(e=>setNotice("Customer display test failed: "+e.message));
 ...
 return <section className="settings-page"><div className="settings-shell">
  ...
  <div className="settings-main">
   ...
   {tab==="Print"&&<><h2>Printer selection</h2><div className="settings-grid"><Field label="Print receipt"><Toggle checked={pr.printReceipt!==false} onChange={v=>patch("print","printReceipt",v)}/></Field><Field label="Print credit payments"><Toggle checked={pr.printCreditPayments} onChange={v=>patch("print","printCreditPayments",v)}/></Field><Field label="Print locked sale"><Toggle checked={pr.printLockedSale} onChange={v=>patch("print","printLockedSale",v)}/></Field><Field label="Print kitchen ticket"><Toggle checked={pr.printKitchenTicket} onChange={v=>patch("print","printKitchenTicket",v)}/></Field><Field label="Print service messages"><Toggle checked={pr.printServiceMessages} onChange={v=>patch("print","printServiceMessages",v)}/></Field><Field label="Printer"><select value={pr.printer||""} onChange={e=>patch("print","printer",e.target.value)}><option value="">Select printer</option>{printers.map(x=><option key={x.name} value={x.name}>{x.name}</option>)}</select></Field><Field label="Printer type"><select value={pr.printerType||"Windows printer"} onChange={e=>patch("print","printerType",e.target.value)}><option>Windows printer</option><option>Generic / Text only</option><option>Standard A4</option></select></Field><Field label="Paper size"><select value={pr.paperSize||"80 mm"} onChange={e=>patch("print","paperSize",e.target.value)}><option>58 mm</option><option>80 mm</option><option>A4</option></select></Field><Field label="Number of copies"><input type="number" min="1" value={pr.copies??1} onChange={e=>patch("print","copies",Math.max(1,Number(e.target.value)))}/></Field><Field label="Characters per line"><input type="number" min="20" value={pr.charactersPerLine??42} onChange={e=>patch("print","charactersPerLine",Number(e.target.value))}/></Field><Field label="Right to left"><Toggle checked={pr.rightToLeft} onChange={v=>patch("print","rightToLeft",v)}/></Field><Field label="Feed lines"><input type="number" min="0" max="20" value={pr.feedLines??3} onChange={e=>patch("print","feedLines",Math.max(0,Number(e.target.value)))}/></Field><Field label="Cut paper"><Toggle checked={pr.cutPaper} onChange={v=>patch("print","cutPaper",v)}/></Field><Field label="Print bitmap"><Toggle checked={pr.printBitmap} onChange={v=>patch("print","printBitmap",v)}/></Field><Field label="Rich formatting"><Toggle checked={pr.richFormatting} onChange={v=>patch("print","richFormatting",v)}/></Field><Field label="Print barcode"><Toggle checked={pr.printBarcode} onChange={v=>patch("print","printBarcode",v)}/></Field><Field label="Print logo full width"><Toggle checked={pr.printLogoFullWidth} onChange={v=>patch("print","printLogoFullWidth",v)}/></Field><Field label="Alignment"><select value={pr.alignment||"Left"} onChange={e=>patch("print","alignment",e.target.value)}><option>Left</option><option>Center</option></select></Field><Field label="Code page"><input value={pr.codePage||"437"} onChange={e=>patch("print","codePage",e.target.value)}/></Field><Field label="Character set"><select value={pr.characterSet||"None"} onChange={e=>patch("print","characterSet",e.target.value)}>{["None","USA","France","Germany","UK","Denmark I","Sweden","Italy","Spain I","Japan","Norway","Denmark II","Spain II","Latin America","Korea","Slovenia / Croatia","China","Vietnam","Arabia"].map(x=><option key={x}>{x}</option>)}</select></Field></div><h2>Margins (millimeters)</h2><div className="settings-grid"><Field label="Top"><input type="number" value={pr.marginTop??0} onChange={e=>patch("print","marginTop",Number(e.target.value))}/></Field><Field label="Right"><input type="number" value={pr.marginRight??0} onChange={e=>patch("print","marginRight",Number(e.target.value))}/></Field><Field label="Bottom"><input type="number" value={pr.marginBottom??0} onChange={e=>patch("print","marginBottom",Number(e.target.value))}/></Field><Field label="Left"><input type="number" value={pr.marginLeft??0} onChange={e=>patch("print","marginLeft",Number(e.target.value))}/></Field></div><h2>Receipt</h2><div className="settings-grid"><Field label="Header" wide><textarea value={pr.header||""} onChange={e=>patch("print","header",e.target.value)}/></Field><Field label="Footer" wide><textarea value={pr.footer||""} onChange={e=>patch("print","footer",e.target.value)}/></Field><Field label="Invoice title"><input value={pr.invoiceTitle||""} onChange={e=>patch("print","invoiceTitle",e.target.value)}/></Field><Field label="Print in A5 size"><Toggle checked={pr.printA5} onChange={v=>patch("print","printA5",v)}/></Field><Field label="Tax column"><Toggle checked={pr.taxColumn} onChange={v=>patch("print","taxColumn",v)}/></Field><Field label="Discount column"><Toggle checked={pr.discountColumn} onChange={v=>patch("print","discountColumn",v)}/></Field><Field label="Customer tax number"><Toggle checked={pr.customerTaxNumber} onChange={v=>patch("print","customerTaxNumber",v)}/></Field><Field label="Customer code"><Toggle checked={pr.customerCode} onChange={v=>patch("print","customerCode",v)}/></Field><Field label="Customer phone"><Toggle checked={pr.customerPhone} onChange={v=>patch("print","customerPhone",v)}/></Field><Field label="Customer email"><Toggle checked={pr.customerEmail} onChange={v=>patch("print","customerEmail",v)}/></Field><Field label="Payment methods"><Toggle checked={pr.paymentTypes} onChange={v=>patch("print","paymentTypes",v)}/></Field><Field label="Outstanding balance"><Toggle checked={pr.outstandingBalance} onChange={v=>patch("print","outstandingBalance",v)}/></Field><Field label="Font family"><input value={pr.fontFamily||"Arial"} onChange={e=>patch("print","fontFamily",e.target.value)}/></Field><Field label="Font size (%)"><input type="number" min="50" max="200" value={pr.fontSize??100} onChange={e=>patch("print","fontSize",Number(e.target.value))}/></Field></div><div className="settings-database-actions"><button onClick={testPrinter}>▣ Print test page</button></div><p className="settings-help">Some printer options are configuration values until the selected Windows/ESC-POS printer exposes matching capabilities through the Local Agent.</p></>}
   {tab==="Hardware"&&<><h2>Local hardware</h2><p className="settings-help">SP-Manager Local Agent connects the browser to Windows printers, cash drawers and serial customer displays.</p><div className="hardware-status-card"><div className="hardware-status-head"><div><b>SP-Manager Local Agent</b><span className="settings-help">Hardware bridge</span></div><button onClick={refreshHardware}>↻ Refresh</button></div>{hardwareStatus?.connected?<div className="hardware-status-state ok"><span className="hardware-dot">●</span><div><strong>CONNECTED</strong><small>Agent is running on this POS computer · v{hardwareStatus.version||"1.0.5"}</small></div></div>:<div className="hardware-status-state error"><span className="hardware-dot">●</span><div><strong>AGENT NOT DETECTED</strong><small>Local Agent is not reachable.</small></div></div>}<div className="hardware-status-hint">Install/start the Local Agent on the Windows POS computer, then press Refresh.</div></div><div className="settings-grid"><Field label="Local Agent enabled"><Toggle checked={!!hw.agentEnabled} onChange={v=>patch("hardware","agentEnabled",v)}/></Field><Field label="Local Agent URL"><input value={hw.agentUrl||"http://127.0.0.1:18765"} onChange={e=>patch("hardware","agentUrl",e.target.value)}/></Field><Field label="Windows receipt printer"><select value={hw.printer||""} onChange={e=>patch("hardware","printer",e.target.value)}><option value="">Select printer</option>{printers.map(x=><option key={x.name} value={x.name}>{x.name}</option>)}</select></Field><Field label="Cash drawer enabled"><Toggle checked={!!hw.cashDrawerEnabled} onChange={v=>patch("hardware","cashDrawerEnabled",v)}/></Field><Field label="Cash drawer printer"><select value={hw.cashDrawerPrinter||""} onChange={e=>patch("hardware","cashDrawerPrinter",e.target.value)}><option value="">Use receipt printer</option>{printers.map(x=><option key={"d"+x.name} value={x.name}>{x.name}</option>)}</select></Field><Field label="Cash drawer command"><input value={(hw.cashDrawerPulse||[27,112,0,25,250]).join(",")} onChange={e=>patch("hardware","cashDrawerPulse",e.target.value.split(",").map(x=>Number(x.trim())).filter(x=>Number.isFinite(x)))}/></Field><Field label="Customer display enabled"><Toggle checked={!!hw.customerDisplayEnabled} onChange={v=>patch("hardware","customerDisplayEnabled",v)}/></Field></div><div className="settings-database-actions"><button onClick={testPrinter}>▣ Test printer</button><button onClick={testDrawer}>▤ Test cash drawer</button><button onClick={testDisplay}>▤ Test customer display</button></div></>}
