 ...
 return <div className="price-tags-overlay">
  <div className="price-tags-shell" onMouseDown={e=>e.stopPropagation()}>
   ...
   <div className="price-tags-body">
    <aside className="price-tags-settings sp-manager-price-settings">
    ...
    <main className="price-tags-preview-area">
     <div className="pt-preview-toolbar sp-manager-preview-toolbar">
...
function Products({products,setProducts,addProduct,updateProduct,editing,setEditing,categories,setCategories,productGroups,setProductGroups,groupMeta,setGroupMeta,suppliers=[],setNotice,settings=defaultSettings}){
 ...
 const groupDescendants=name=>{const out=[];const seen=new Set([name]);const walk=n=>{childrenOf(n).forEach(c=>{if(seen.has(c))return;seen.add(c);out.push(c);walk(c)})};walk(name);return out};
 ...
 const visible=products.filter(p=>{
  ...
  const g=selectedGroup==="All Groups"||p.group===selectedGroup||groupDescendants(selectedGroup).includes(p.group);
 ...
 const renderGroupTree=(name,depth=0,trail=new Set())=>{if(trail.has(name))return null;const nextTrail=new Set(trail);nextTrail.add(name);const kids=childrenOf(name);const expanded=expandedCategories.has(name);return <div key={name} className="tree-category"><button className={selectedGroup===name?"tree-item selected":"tree-item"} style={{paddingLeft:12+depth*18}} onClick={()=>{setSelectedCategory("All Products");setSelectedGroup(name);setExpandedCategories(a=>{const n=new Set(a);n.has(name)?n.delete(name):n.add(name);return n})}}>{kids.length?(expanded?"▾":"▸"):"▪"} <span className="tree-label-with-image">{groupMeta?.[name]?.image?<img src={groupMeta[name].image} alt=""/>:null}<span>{name}</span></span><em>{groupProductCount(name)}</em></button>{expanded&&kids.map(child=>renderGroupTree(child,depth+1,nextTrail))}</div>};
 return <section className="content product-master-modern">
  ...
  {showGroup&&<div className="modal-backdrop" onMouseDown={()=>setShowGroup(false)}><form className="modal group-editor-modal" onSubmit={saveGroup} onMouseDown={e=>e.stopPropagation()}><div className="modal-head"><div><span className="eyebrow">PRODUCT GROUPS</span><h3>New {groupFormLevel}</h3></div><button type="button" className="iconbtn" onClick={()=>setShowGroup(false)}>×</button></div><div className="group-editor-grid"><label>Group name<input autoFocus value={groupForm.name} onChange={e=>{setGroupForm(f=>({...f,name:e.target.value}));setGroupError("")}} placeholder="Enter group name"/></label><label>Parent group<select value={groupForm.parent} onChange={e=>setGroupForm(f=>({...f,parent:e.target.value}))}><option value="">Top level group</option>{groups.map(g=><option key={g} value={g}>{g}</option>)}</select></label><div className="group-image-field"><span>Group image</span><div className="group-image-row">{(groupImageFile||groupForm.image)?<img src={groupImageFile||groupForm.image} alt=""/>:<div className="group-image-empty">No image</div>}<div><input id="new-group-image" type="file" accept="image/*" onChange={handleGroupImage}/><button type="button" className="secondary smallbtn" onClick={()=>{setGroupImageFile("");setGroupForm(f=>({...f,image:""}));const el=document.getElementById("new-group-image");if(el)el.value=""}}>Clear image</button></div></div><small>Image is optional. SP-Manager-compatible group image is stored with the group.</small></div></div>{groupError&&<p className="modal-inline-error">{groupError}</p>}<div className="group-editor-actions"><button type="button" className="secondary" onClick={()=>setShowGroup(false)}>Cancel</button><button type="submit">Create group</button></div></form></div>}
  ...
  <div className="settings-main">
   ...
   {tab==="Print"&&<div className="print-settings-module">
    ...
    {printTab==="Customize receipt"&&<div className="print-settings-content">
      <div className="print-info-banner"><span>ⓘ</span><div>Receipt formatting and customer-detail options, following the SP-Manager Print Options structure.</div></div>
    ...
    {printTab==="Print templates"&&<div className="print-settings-content">
      <div className="print-info-banner"><span>ⓘ</span><div>The selected font is used in A4 invoice and report templates. Invoice options below follow the SP-Manager Print templates structure.</div></div>
