...
const buildInvoiceHtml=(sale,company={},customers=[],settings={})=>{
 ...
 return `<div class="invoice-page">
  <div class="invoice-head"><div class="invoice-title">${escapeHtml(settings?.print?.invoiceTitle||"INVOICE")}</div><div class="invoice-company"><div class="invoice-company-top">${logo}</div>${companyName?`<div class="company-name">${escapeHtml(companyName)}</div>`:""}${addr?`<div class="company-address">${escapeHtml(addr)}</div>`:""}${companyPhone?`<div class="company-contact">${escapeHtml(companyPhone)}</div>`:""}${companyEmail?`<div class="company-contact">${escapeHtml(companyEmail)}</div>`:""}<div class="invoice-meta"><div><b>Invoice No.:</b> ${escapeHtml(sale?.no||"")}</div><div><b>Date:</b> ${escapeHtml(formatInvoiceDate(sale?.date))}</div><div><b>Due date:</b> ${escapeHtml(formatInvoiceDate(dueDate))}</div><div><b>Payment status:</b> ${sale?.paid===false||due>0?"Unpaid":"Paid"}</div></div></div></div>
