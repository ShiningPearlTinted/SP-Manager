...
function POS({setCart,updateLinePrice,posOrderMeta,setPosOrderMeta,retrieveOpenOrder,filtered,q,setQ,posSearchMode,setPosSearchMode,add,cart,changeQty,customers,setCustomers,customer,setCustomer,discount,setDiscount,discountFixed,setDiscountFixed,payment,setPayment,paymentTypes,subtotal,disc,taxRate,setTaxRate,tax,grand,sale,saveOpenOrder,orders,setOrders,occupiedTables,updateSaleNote,setNoteBox,clearCurrentSale,printReceipt,closeLastSale,categories,posCategory,setPosCategory,products,company,lastSale,menuOpen,setMenuOpen,setPage,sales,emailReceipt,settings,activeUser,signOut,openCashInOut,openCashDrawer,openEndOfDayFromPOS,groupMeta}){
 ...
 const categoryTile=name=><button type="button" className="ar-category-tile" key={name} onClick={()=>{setPosCategory(name);setGroup("");setCatLevel("group")}}><div className="ar-cat-icon">{groupMeta?.[name]?.image?<img className="group-tile-image" src={groupMeta[name].image} alt=""/>:<span>{iconFor(name)}</span>}</div><strong>{name}</strong><small>{products.filter(p=>(p.category||p.group)===name).length} products</small></button>;
 const groupTile=name=><button type="button" className="ar-category-tile ar-group-tile" key={name} onClick={()=>{setGroup(name);setCatLevel("items")}}><div className="ar-cat-icon">{groupMeta?.[name]?.image?<img className="group-tile-image" src={groupMeta[name].image} alt=""/>:<span>{iconFor(name)}</span>}</div><strong>{name}</strong><small>{categoryProducts.filter(p=>(p.group||p.category)===name).length} products</small></button>;
...
function Products({products,setProducts,addProduct,updateProduct,editing,setEditing,categories,setCategories,productGroups,setProductGroups,groupMeta,setGroupMeta,suppliers=[],setNotice,settings=defaultSettings}){
 ...
 const saveEditGroup=()=>{const name=String(groupForm.name||"").trim();if(!name){setGroupError("Enter a product group name.");return}const oldName=selectedGroup;if(name!==oldName&&groups.some(g=>g.toLowerCase()===name.toLowerCase())){setGroupError("This product group already exists.");return}const parent=groupFormLevel==="Second group"?String(groupForm.parent||"").trim():"";if(groupFormLevel==="Second group"&&!parent){setGroupError("Select a main group.");return}if(groupFormLevel==="Second group"&&childrenOf(oldName).length){setGroupError("This main group has second group(s). Move or delete them before changing its level.");return}if(parent===oldName||groupDescendants(oldName).includes(parent)){setGroupError("Invalid parent group.");return}setGroupConfirm({type:"edit",oldName,name,parent,image:groupImageFile||groupForm.image||""});};
 ...
 const saveGroup=e=>{e?.preventDefault();const name=String(groupForm.name||"").trim();if(!name){setGroupError("Enter a product group name.");return}if(groups.some(g=>g.toLowerCase()===name.toLowerCase())){setGroupError("This product group already exists.");return}const parent=groupFormLevel==="Second group"?String(groupForm.parent||"").trim():"";if(groupFormLevel==="Second group"&&!parent){setGroupError("Select a main group.");return}setGroupConfirm({type:"save",name,parent,image:groupImageFile||""});};
 const confirmGroupAction=()=>{if(!groupConfirm)return;const {type,name,parent,image,oldName}=groupConfirm;if(type==="save"){const next=[...groups,name];const categoryMap={...groupCategories,...(parent?{[name]:parent}:{})};const meta={...groupMeta,[name]:{parent:parent||"",image:image||"",rank:groups.length+1}};save("productGroups",next);save("productGroupCategories",categoryMap);save("productGroupMeta",meta);setProductGroups(next);setGroupCategories(categoryMap);setGroupMeta(meta);setNewGroup("");setGroupError("");setShowGroup(false);setForm(f=>({...f,group:name,category:parent||f.category}));setSelectedGroup(name);if(parent)setExpandedCategories(a=>new Set(a).add(parent));setNotice((parent?"Second group":"Main group")+" added successfully.")}if(type==="edit"){const next=groups.map(g=>g===oldName?name:g);const np=products.map(p=>{let q=p;if(p.group===oldName)q={...q,group:name,updatedAt:new Date().toISOString()};if(p.category===oldName&&!parent)q={...q,category:name,updatedAt:new Date().toISOString()};return q});const meta={...groupMeta};const oldMeta=meta[oldName]||{};delete meta[oldName];meta[name]={parent:parent||"",image:image||oldMeta.image||"",rank:oldMeta.rank||0};Object.keys(meta).forEach(k=>{if(meta[k]?.parent===oldName)meta[k]={...meta[k],parent:name}});save("productGroups",next);save("products",np);save("productGroupMeta",meta);setProductGroups(next);setProducts(np);setGroupMeta(meta);setSelectedGroup(name);setShowEditGroup(false);setNotice("Product group updated successfully.")}if(type==="delete"){const next=groups.filter(g=>g!==name);const meta={...groupMeta};delete meta[name];save("productGroups",next);save("productGroupMeta",meta);setProductGroups(next);setGroupMeta(meta);setSelectedGroup("All Groups");setNotice("Product group deleted successfully.")}setGroupConfirm(null)};
 ...
 return <section className="content product-master-modern">
  ...
  <aside className="product-tree"><div className="tree-title">Product Groups</div><button className={selectedGroup==="All Groups"?"tree-item selected":"tree-item"} onClick={()=>{setSelectedCategory("All Products");setSelectedGroup("All Groups")}}>▣ <span>All Products</span><em>{products.length}</em></button>{mainGroups.map(main=>{const kids=childrenOf(main);const expanded=expandedCategories.has(main);return <div key={main} className="tree-category"><button className={selectedGroup===main?"tree-item selected":"tree-item"} onClick={()=>{setSelectedCategory("All Products");setSelectedGroup(main);setExpandedCategories(a=>{const n=new Set(a);n.has(main)?n.delete(main):n.add(main);return n})}}>{expanded?"▾":"▸"} <span className="tree-label-with-image">{groupMeta?.[main]?.image?<img src={groupMeta[main].image} alt=""/>:null}<span>{main}</span></span><em>{groupProductCount(main)}</em></button>{expanded&&kids.map(g=><button key={g} className={selectedGroup===g?"tree-item child selected":"tree-item child"} onClick={()=>{setSelectedCategory("All Products");setSelectedGroup(g)}}>▪ <span className="tree-label-with-image">{groupMeta?.[g]?.image?<img src={groupMeta[g].image} alt=""/>:null}<span>{g}</span></span><em>{groupProductCount(g)}</em></button>)}</div>})}</aside>
  ...
  {showGroup&&<div className="modal-backdrop" onMouseDown={()=>setShowGroup(false)}><form className="modal group-editor-modal" onSubmit={saveGroup} onMouseDown={e=>e.stopPropagation()}><div className="modal-head"><div><span className="eyebrow">PRODUCT GROUPS</span><h3>New {groupFormLevel}</h3></div><button type="button" className="iconbtn" onClick={()=>setShowGroup(false)}>×</button></div><div className="group-editor-grid"><label>Group level<select value={groupFormLevel} onChange={e=>{setGroupFormLevel(e.target.value);setGroupForm(f=>({...f,parent:e.target.value==="Second group"?(f.parent||mainGroups[0]||""):""}));setGroupError("")}}><option>Main group</option><option>Second group</option></select></label><label>Group name<input autoFocus value={groupForm.name} onChange={e=>{setGroupForm(f=>({...f,name:e.target.value}));setGroupError("")}} placeholder="Enter group name"/></label>{groupFormLevel==="Second group"&&<label>Main group<select value={groupForm.parent} onChange={e=>setGroupForm(f=>({...f,parent:e.target.value}))}><option value="">Select main group</option>{mainGroups.map(g=><option key={g} value={g}>{g}</option>)}</select></label>}<div className="group-image-field"><span>Group image</span><div className="group-image-row">{(groupImageFile||groupForm.image)?<img src={groupImageFile||groupForm.image} alt=""/>:<div className="group-image-empty">No image</div>}<div><input id="new-group-image" type="file" accept="image/*" onChange={handleGroupImage}/><button type="button" className="secondary smallbtn" onClick={()=>{setGroupImageFile("");setGroupForm(f=>({...f,image:""}));const el=document.getElementById("new-group-image");if(el)el.value=""}}>Clear image</button></div></div><small>Image is optional. Aronium-compatible group image is stored with the group.</small></div></div>{groupError&&<p className="modal-inline-error">{groupError}</p>}<div className="group-editor-actions"><button type="button" className="secondary" onClick={()=>setShowGroup(false)}>Cancel</button><button type="submit">Create group</button></div></form></div>}
  ...
  {showEditGroup&&<div className="modal-backdrop" onMouseDown={()=>setShowEditGroup(false)}><form className="modal group-editor-modal" onSubmit={e=>{e.preventDefault();saveEditGroup()}} onMouseDown={e=>e.stopPropagation()}><div className="modal-head"><div><span className="eyebrow">PRODUCT GROUPS</span><h3>Edit {groupFormLevel}</h3></div><button type="button" className="iconbtn" aria-label="Close" onClick={()=>setShowEditGroup(false)}>×</button></div><div className="group-editor-grid"><label>Group level<select value={groupFormLevel} onChange={e=>{setGroupFormLevel(e.target.value);setGroupForm(f=>({...f,parent:e.target.value==="Second group"?(f.parent||mainGroups.find(g=>g!==selectedGroup)||""):""}));setGroupError("")}}><option>Main group</option><option>Second group</option></select></label><label>Group name<input autoFocus value={groupForm.name} onChange={e=>{setGroupForm(f=>({...f,name:e.target.value}));setGroupError("")}}/></label>{groupFormLevel==="Second group"&&<label>Main group<select value={groupForm.parent} onChange={e=>setGroupForm(f=>({...f,parent:e.target.value}))}><option value="">Select main group</option>{mainGroups.filter(g=>g!==selectedGroup&&!groupDescendants(selectedGroup).includes(g)).map(g=><option key={g} value={g}>{g}</option>)}</select></label>}<div className="group-image-field"><span>Group image</span><div className="group-image-row">{(groupImageFile||groupForm.image)?<img src={groupImageFile||groupForm.image} alt=""/>:<div className="group-image-empty">No image</div>}<div><input id="edit-group-image" type="file" accept="image/*" onChange={handleGroupImage}/><button type="button" className="secondary smallbtn" onClick={()=>{setGroupImageFile("");setGroupForm(f=>({...f,image:""}));const el=document.getElementById("edit-group-image");if(el)el.value=""}}>Clear image</button></div></div><small>Image is optional. The same image is used in the product-group tree and POS group tile.</small></div></div>{groupError&&<p className="modal-inline-error">{groupError}</p>}<div className="group-editor-actions"><button type="button" className="secondary" onClick={()=>setShowEditGroup(false)}>Cancel</button><button type="submit">Save changes</button></div></form></div>}
 </section>
}
...
function Settings({settings,setSettings,businessDay,toggleBusiness,taxRate,setTaxRate,company,onCancel}){
 ...
 const prt=pr;
 const [hardwareStatus,setHardwareStatus]=useState(null); const [printers,setPrinters]=useState([]); const [hardwareRefreshing,setHardwareRefreshing]=useState(false);
