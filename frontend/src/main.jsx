...
function POS({updateLinePrice,posOrderMeta,setPosOrderMeta,retrieveOpenOrder,filtered,q,setQ,posSearchMode,setPosSearchMode,add,cart,changeQty,customers,setCustomers,customer,setCustomer,discount,setDiscount,discountFixed,setDiscountFixed,payment,setPayment,paymentTypes,subtotal,disc,taxRate,setTaxRate,tax,grand,sale,saveOpenOrder,orders,setOrders,updateSaleNote,setNoteBox,clearCurrentSale,printReceipt,closeLastSale,categories,posCategory,setPosCategory,products,company,lastSale,menuOpen,setMenuOpen,setPage,sales,emailReceipt,settings,activeUser,signOut,openCashInOut,openCashDrawer,openEndOfDayFromPOS}){
 ...
 const pushCustomerDisplay=async()=>{const cd=settings?.customerDisplay||{};const hw=settings?.hardware||{};if(!cd.enabled&&!hw.customerDisplayEnabled)return;const base=String(hw.agentUrl||"http://127.0.0.1:18765").replace(/\/$/,"");const c=customers.find(x=>x.id===customer);const item=cart[cart.length-1];try{await fetch(base+"/display",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({port:cd.comPort||hw.customerDisplayPort,baud:Number(cd.baudRate||hw.customerDisplayBaud||9600),line1:cart.length?(item?.name||"TOTAL"):((c?.name||"Walk-in Customer").slice(0,Number(cd.characters||20))),line2:cart.length?money(grand):String(cd.bottomLine||""),chars:Number(cd.characters||hw.customerDisplayChars||20)})})}catch{}};
 ...
 return <section className={"ar-pos-shell "+(posFullscreen?"pos-browser-fullscreen":"")}>
  <div className="ar-topbar">
   {[['⌕','Search'],['♙','Customer'],['⇄','Transfer'],['%','Discount'],['＋','New sale'],['↶','Refund'],['✎','Comment'],['◉','Service type'],['#','Order name'],['▤','Cash drawer'],['F9','Save sale'],['F10','Payment'],...enabledPayments.filter(x=>x.quickPayment).map(x=>['PAYMENT_ICON',String(x.name).toLowerCase()==='cash'?'F12 Cash':x.name])].filter(([,label])=>{const m={Search:"search",Customer:"customer",Transfer:"transfer",Discount:"discount","New sale":"newSale",Refund:"refund",Comment:"comment","Service type":"serviceType","Order name":"orderName","Cash drawer":"cashDrawer","Save sale":"newSale",Payment:"payment"};const key=label.replace(/^F10 /,"").replace(/^F12 .*/,"Cash");return m[key]===undefined||settings.general.buttonBar[m[key]]!==false||label.includes(" ")}).map(([ic,label])=><button key={label} className={'ar-action '+(label==='F10 Payment'?'selected':'')} onClick={()=>{
     ...
     if(label==='Order name'){if(saleLocked){setNoticeLocal('Sale is locked. Unlock the sale before changing order name.');return}setOrderNameValue(posOrderMeta?.name||'');setOrderNameSaveMode(false);setOrderNameScreen(true);return}
