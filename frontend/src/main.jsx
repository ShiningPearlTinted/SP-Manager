...
const seedProducts=[
{id:1,code:"SP001",name:"Tinted Film Standard",group:"Tinted Film",price:180,cost:90,stock:12,reorder:5},
{id:2,code:"SP002",name:"Tinted Film Premium",group:"Tinted Film",price:280,cost:140,stock:8,reorder:5},
{id:3,code:"SP003",name:"Tinted Film Ceramic",group:"Tinted Film",price:450,cost:220,stock:5,reorder:5},
{id:4,code:"SP004",name:"Front Windscreen Film",group:"Windscreen",price:160,cost:80,stock:14,reorder:5},
{id:5,code:"SP005",name:"Rear Windscreen Film",group:"Windscreen",price:140,cost:70,stock:10,reorder:5},
{id:6,code:"SP006",name:"Door Glass Film",group:"Glass",price:95,cost:45,stock:20,reorder:5},
{id:7,code:"SP007",name:"Security Film",group:"Security",price:520,cost:260,stock:4,reorder:5},
{id:8,code:"SP008",name:"UV Protection Film",group:"Protection",price:320,cost:160,stock:9,reorder:5}
];
const seedCustomers=[
...
function App(){
 ...
 const completeSale=()=>{
  ...
  setCart([]);setDiscount(0);setNotice("Jualan berjaya: "+sale.no+" — "+money(grand));setPage("POS / Sales");
 };
 const refund=id=>{
  ...
  persist("sales",ns,setSales);persist("products",np,setProducts);setNotice("Refund berjaya untuk "+s.no);
 };
 const voidSale=id=>{
  const s=sales.find(x=>x.id===id);if(!s||s.voided||s.refunded)return;
  const ns=sales.map(x=>x.id===id?{...x,voided:true}:x);
  const np=products.map(p=>{const i=s.items.find(x=>x.id===p.id);return i?{...p,stock:p.stock+i.qty}:p});
  persist("sales",ns,setSales);persist("products",np,setProducts);setNotice("Transaksi "+s.no+" telah void.");
 };
 const addProduct=p=>{
  const np=[...products,{...p,id:uid(),price:Number(p.price),cost:Number(p.cost),stock:Number(p.stock),reorder:Number(p.reorder)}];
  persist("products",np,setProducts);setNotice("Produk ditambah.");
 };
 const updateProduct=p=>{
  const np=products.map(x=>x.id===p.id?{...p}:x);persist("products",np,setProducts);setEditing(null);setNotice("Produk dikemaskini.");
 };
 const addCustomer=c=>{
  const nc=[...customers,{...c,id:uid(),visits:0,spend:0}];persist("customers",nc,setCustomers);setNotice("Customer ditambah.");
 };
 const receivePurchase=(supplierId,items,total)=>{
  ...
  persist("purchases",[...purchases,po],setPurchases);persist("products",np,setProducts);setNotice("Purchase diterima dan stock bertambah.");
 };
 const savePromo=p=>{const np=p.id?promos.map(x=>x.id===p.id?{...p}:x):[...promos,{...p,id:uid()}];persist("promos",np,setPromos);setNotice("Promotion disimpan.")};
 const toggleBusiness=()=>{
  const b={...businessDay,open:!businessDay.open,closedAt:businessDay.open?new Date().toISOString():null};
  persist("businessDay",b,setBusinessDay);setNotice(b.open?"Business day dibuka.":"Business day ditutup.");
 };
 ...
 return <div className="app">
  ...
  <main><header><div><small>SHINING PEARL TINTED</small><h1>{page}</h1></div><div className="head-actions"><span className="day">● {businessDay.open?"Business Day Open":"Closed"}</span><span>● Online</span></div></header>
   ...
   {page==="Products"&&<Products products={products} addProduct={addProduct} updateProduct={updateProduct} editing={editing} setEditing={setEditing}/>}
   ...
   {page==="Tax"&&<Tax rate={taxRate} setRate={r=>{setTaxRate(r);save("taxRate",r);setNotice("Tax rate disimpan.")}}/>}
   {page==="Loyalty"&&<Loyalty customers={customers}/>}
   {page==="Users & Permissions"&&<Users users={users} setUsers={u=>{persist("users",u,setUsers);setNotice("User dikemaskini.")}}/>}
   ...
   {page==="Named Order / Takeaway"&&<NamedOrders orders={orders} setOrders={o=>{persist("orders",o,setOrders);setNotice("Order disimpan.")}} customers={customers}/>}
...
function Dashboard({sales,total,products,lowStock,setPage,businessDay,toggleBusiness}){
 ...
 <div className="grid2"><div className="panel"><h3>Recent Sales</h3>{sales.slice(-7).reverse().map(s=><div className="row" key={s.id}><span><b>{s.no}</b><small>{new Date(s.date).toLocaleString()}</small></span><strong>{money(s.total)}</strong></div>)}{!sales.length&&<Empty text="Belum ada transaksi."/ >}</div>
...
function POS({filtered,q,setQ,add,cart,changeQty,customers,customer,setCustomer,discount,setDiscount,payment,setPayment,subtotal,disc,taxRate,setTaxRate,tax,grand,sale}){
 ...
 {cart.map(i=><div className="cartrow" key={i.id}><span><b>{i.name}</b><small>{money(i.price)} × {i.qty}</small></span><div><button onClick={()=>changeQty(i.id,1)}>+</button><button onClick={()=>changeQty(i.id,-1)}>−</button></div><strong>{money(i.price*i.qty)}</strong></div>)}{!cart.length&&<Empty text="Cart kosong"/>}
...
function Products({products,addProduct,updateProduct,editing,setEditing}){
 const blank={code:"",name:"",group:"",price:0,cost:0,stock:0,reorder:5};
 const[form,setForm]=useState(blank);
 const openEdit=p=>{setEditing(p);setForm(p)};
 const submit=e=>{e.preventDefault();editing?updateProduct(form):addProduct(form);setForm(blank)};
 return <section className="content"><div className="toolbar"><h3>Products / Product Master</h3><button onClick={()=>{setEditing(null);setForm(blank)}}>+ New Product</button></div>
 <div className="grid2"><div className="panel"><h3>{editing?"Edit Product":"Add Product"}</h3><form onSubmit={submit} className="formgrid">{["code","name","group","price","cost","stock","reorder"].map(k=><input key={k} value={form[k]} type={["price","cost","stock","reorder"].includes(k)?"number":"text"} placeholder={k} onChange={e=>setForm({...form,[k]:e.target.value})} required={k==="name"}/>) }<button type="submit">{editing?"Save Changes":"Add Product"}</button></form></div>
 <div className="panel"><Table cols={["Code","Product","Group","Price","Stock","Status",""]} rows={products.map(p=>[p.code,p.name,p.group,money(p.price),p.stock,p.stock<=p.reorder?"Low":"OK",<button onClick={()=>openEdit(p)}>Edit</button>])}/></div></div></section>
}
...
function Reports({sales,products,customers}){const valid=sales.filter(s=>!s.voided&&!s.refunded);const total=valid.reduce((a,s)=>a+s.total,0);const profit=valid.reduce((a,s)=>a+s.items.reduce((x,i)=>x+(i.price-i.cost)*i.qty,0),0);return <section className="content"><div className="cards"><Card t="Sales" v={money(total)}/><Card t="Transactions" v={valid.length}/><Card t="Gross Profit" v={money(profit)}/><Card t="Customers" v={customers.length}/></div><div className="grid2"><div className="panel"><h3>Sales Products</h3><Table cols={["Product","Qty","Sales"]} rows={products.map(p=>{const q=valid.reduce((a,s)=>a+s.items.filter(i=>i.id===p.id).reduce((z,i)=>z+i.qty,0),0);return[p.name,q,money(q*p.price)]})}/></div><div className="panel"><h3>Sales Customers</h3><Table cols={["Customer","Transactions","Spend"]} rows={customers.map(c=>{const ss=valid.filter(s=>s.customerId===c.id);return[c.name,ss.length,money(ss.reduce((a,s)=>a+s.total,0))]})}/></div></div></section>}
