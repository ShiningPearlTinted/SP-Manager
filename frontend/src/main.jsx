...
const buildInvoiceHtml=(sale,company={},customers=[],settings={})=>{
 ...
 const addr=invoiceAddress(company).replace(/,\s*Malaysia$/i,"");
 ...
 const itemRows=items.map((i,n)=>`<tr><td>${n+1}</td><td>${escapeHtml(i.name||"")}</td><td class="num">${escapeHtml(i.qty||0)}</td><td class="num">${escapeHtml(Number(i.price||0).toFixed(2))}</td><td class="num">---</td><td class="num">0.00%</td><td class="num">${escapeHtml(money(Number(i.price||0)*Number(i.qty||0)))}</td></tr>`).join("");
 const publicNote=String(sale?.note||"").trim();
 return `<div class="invoice-page">
  <div class="invoice-head"><div class="invoice-company-info"><div class="invoice-title">${escapeHtml(settings?.print?.invoiceTitle||"INVOICE")}</div>${companyName?`<div class="company-name">${escapeHtml(companyName)}</div>`:""}${addr?`<div class="company-address">${escapeHtml(addr)}</div>`:""}${companyPhone?`<div class="company-contact">${escapeHtml(companyPhone)}</div>`:""}${companyEmail?`<div class="company-contact">${escapeHtml(companyEmail)}</div>`:""}</div><div class="invoice-company"><div class="invoice-company-top">${logo}</div><div class="invoice-meta"><div><b>Invoice No.:</b> ${escapeHtml(sale?.no||"")}</div><div><b>Date:</b> ${escapeHtml(formatInvoiceDate(sale?.date))}</div><div><b>Due date:</b> ${escapeHtml(formatInvoiceDate(dueDate))}</div><div><b>Payment status:</b> ${sale?.paid===false||due>0?"Unpaid":"Paid"}</div></div></div></div>
  <div class="invoice-info-grid"><div><b>Bill to</b><div>${escapeHtml(customer?.name||"Walk-in customer")}</div>${settings?.print?.customerPhone&&customer?.phone&&customer.phone!=="-"?`<div>${escapeHtml(customer.phone)}</div>`:""}${settings?.print?.customerEmail&&customer?.email&&customer.email!=="-"?`<div>${escapeHtml(customer.email)}</div>`:""}</div><div></div></div>
  <table class="invoice-table"><thead><tr><th>#</th><th>Item</th><th>Quantity</th><th>Unit price</th><th>Tax</th><th>Discount</th><th>Total</th></tr></thead><tbody>${itemRows||`<tr><td colspan="7">No items</td></tr>`}</tbody></table>
  <div class="invoice-total"><span>Total</span><b>${escapeHtml(money(total))}</b></div>
  <div class="payment-summary"><div class="payment-heading"><b>Payment method:</b></div>${paymentLines}<div><span>Paid amount:</span><b>${escapeHtml(money(paid))}</b></div><div><span>Amount due:</span><b>${escapeHtml(money(due))}</b></div></div>
...
const invoiceCss=`@page{size:A4;margin:0}body{font-family:Arial,Segoe UI,sans-serif;color:#111;background:#fff;margin:0}.invoice-page{box-sizing:border-box;width:210mm;min-height:297mm;padding:12mm 10mm 12mm;margin:auto;font-size:10px}.invoice-head{display:flex;justify-content:space-between;align-items:flex-start;border-bottom:1px solid #aaa;padding-bottom:5mm;gap:10mm}.invoice-company-info{flex:1;min-width:0;text-align:left}.invoice-company{display:flex;flex-direction:column;align-items:flex-end;justify-content:flex-start;width:78mm;min-width:78mm;max-width:78mm;text-align:right}.invoice-company-top{width:78mm;display:flex;justify-content:flex-end;align-items:flex-start}.company-logo{width:38mm;max-height:20mm;object-fit:contain;object-position:right center}.company-name{margin-top:5mm;font-size:9px;font-weight:700;line-height:1.35}.company-address{max-width:100mm;margin-top:1mm;font-size:9px;font-weight:400;line-height:1.35;overflow-wrap:anywhere}.company-contact{font-size:9px;line-height:1.35}.invoice-meta{width:78mm;margin-top:2.5mm;text-align:right;line-height:1.5}.invoice-meta div{white-space:nowrap}.invoice-title{font-size:17px;font-weight:700;letter-spacing:.4px;margin-bottom:5mm}.invoice-info-grid{display:grid;grid-template-columns:minmax(0,1fr) 88mm;gap:10mm;margin-top:5mm;line-height:1.55}.invoice-info-grid>div:last-child{justify-self:end;width:88mm}.invoice-table{width:100%;border-collapse:collapse;margin-top:5mm;font-size:9px}.invoice-table th{background:#fff;border:1px solid #aaa;padding:4px 4px;text-align:left}.invoice-table td{border:1px solid #ccc;padding:4px 4px;vertical-align:top}.invoice-table th:nth-child(1),.invoice-table td:nth-child(1){width:5%}.invoice-table th:nth-child(3),.invoice-table td:nth-child(3){width:11%}.invoice-table th:nth-child(4),.invoice-table td:nth-child(4){width:14%}.invoice-table th:nth-child(5),.invoice-table td:nth-child(5){width:11%}.invoice-table th:nth-child(6),.invoice-table td:nth-child(6){width:13%}.invoice-table th:nth-child(7),.invoice-table td:nth-child(7){width:15%}.num{text-align:right}.invoice-total{margin-top:3mm;margin-left:auto;width:62mm;display:flex;justify-content:space-between;border:1px solid #aaa;padding:4px 6px;font-weight:700}.payment-summary{margin-top:5mm;margin-left:auto;width:72mm;line-height:1.65}.payment-summary>div{display:flex;justify-content:space-between;gap:8px}.payment-summary .payment-heading{display:block;margin-bottom:1mm}.invoice-note{margin-top:8mm;border-top:1px solid #aaa;padding-top:3mm;line-height:1.55}.invoice-note-title{font-weight:700;margin-bottom:1mm}.invoice-note-text{white-space:normal}.invoice-footer{margin-top:15mm;text-align:right;font-size:8px;color:#666}@media print{.invoice-page{margin:0}}`;
