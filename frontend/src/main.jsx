...
const defaultSettings={
 ...
 customerDisplay:{enabled:false,secondaryMonitor:false,comPort:"",baudRate:9600,dataBits:8,stopBits:1,characters:20,topLine:"WELCOME!",bottomLine:"",displayImage:""},
...
function App(){
 ...
 return <div className={"app app-theme-"+String(settings.general.colorScheme||"Light").toLowerCase().replace(/\s+/g,"-")} dir={settings.general.direction||"ltr"} style={{zoom:Number(settings.general.zoom||100)/100}}>
  ...
  {page==="Loyalty"&&<Loyalty customers={customers} setCustomers={setCustomers}/>}
   ...
   {page==="Settings"&&isPermissionAllowed(activeUser,"manageSettings")&&<Settings settings={settings} setSettings={updateSettings} businessDay={businessDay} toggleBusiness={toggleBusiness} taxRate={taxRate} setTaxRate={r=>{setTaxRate(r);save("taxRate",r)}} company={company} cart={cart} grand={grand} onCancel={()=>setPage("POS / Sales")}/>}
...
function Settings({settings,setSettings,businessDay,toggleBusiness,taxRate,setTaxRate,company,cart=[],grand=0,onCancel}){
 const [tab,setTab]=useState("General");
 ...
 const handleCustomerDisplayImage=event=>{const file=event.target.files?.[0];if(!file)return;if(!file.type.startsWith("image/")){setNotice("Please select an image file.");event.target.value="";return}const reader=new FileReader();reader.onload=()=>{const img=new Image();img.onload=()=>{const maxW=1600,maxH=900;const scale=Math.min(1,maxW/img.naturalWidth,maxH/img.naturalHeight);const canvas=document.createElement("canvas");canvas.width=Math.max(1,Math.round(img.naturalWidth*scale));canvas.height=Math.max(1,Math.round(img.naturalHeight*scale));const ctx=canvas.getContext("2d");ctx.fillStyle="#111";ctx.fillRect(0,0,canvas.width,canvas.height);ctx.drawImage(img,0,0,canvas.width,canvas.height);const data=canvas.toDataURL("image/jpeg",.82);patch("customerDisplay","displayImage",data);setNotice("Customer display image loaded. Click Save to store it.");};img.onerror=()=>setNotice("Unable to read the selected image.");img.src=String(reader.result||"")};reader.onerror=()=>setNotice("Unable to read the selected image.");reader.readAsDataURL(file);event.target.value=""};
 ...
 return <section className="settings-page"><div className="settings-shell">
  ...
  <div className="settings-main">
   ...
   {tab==="Customer display"&&<div className="customer-display-settings-v21"><div className="customer-display-hero"><div><span className="customer-display-eyebrow">CUSTOMER EXPERIENCE</span><h2>Customer Display</h2><p>Show a premium live order screen on the second monitor while the cashier continues using POS normally.</p></div><div className="customer-display-status-pill"><i className={cd.enabled&&cd.secondaryMonitor?"on":""}></i>{cd.enabled&&cd.secondaryMonitor?"SECOND MONITOR ON":"SETUP REQUIRED"}</div></div><div className="customer-display-mode-grid"><div className="customer-display-option"><div><b>Customer display</b><small>Enable live customer-facing output.</small></div><Toggle checked={cd.enabled} onChange={v=>patch("customerDisplay","enabled",v)}/></div><div className="customer-display-option"><div><b>Second monitor</b><small>Open the customer display on Windows monitor 2.</small></div><Toggle checked={cd.secondaryMonitor} onChange={v=>patch("customerDisplay","secondaryMonitor",v)}/></div></div><div className="customer-display-layout-card"><div className="customer-display-layout-preview"><div className="cd-preview-media">{cd.displayImage?<img src={cd.displayImage} alt="Preview"/>:<span>IMAGE<br/><small>50%</small></span>}</div><div className="cd-preview-order"><span>LIVE ORDER</span><b>{cart.length?"Your order":"WELCOME!"}</b><em>{cart.length?`${cart.length} item${cart.length===1?"":"s"}`:"Ready for your order"}</em><strong>{cart.length?money(grand):"RM 0.00"}</strong></div></div><div className="customer-display-layout-copy"><span className="customer-display-eyebrow">SECOND MONITOR</span><h3>50 / 50 Customer Display</h3><p>The left half shows your uploaded promotion or company image. The right half automatically shows the current POS transaction and total.</p><ul><li>Live product, quantity and price updates</li><li>Automatic total update</li><li>Image stays visible during the sale</li></ul></div></div><div className="customer-display-image-card"><div className="customer-display-image-head"><div><span className="customer-display-eyebrow">DISPLAY MEDIA</span><h3>Promotion image</h3><p>Upload an image for the left 50% of the second monitor. It is resized and saved with SP-Manager settings.</p></div>{cd.displayImage&&<button type="button" className="customer-display-remove" onClick={()=>patch("customerDisplay","displayImage","")}>Remove image</button>}</div><label className="customer-display-upload"><input type="file" accept="image/*" onChange={handleCustomerDisplayImage}/><span className="upload-icon">↥</span><span><b>Choose image</b><small>PNG, JPG or WEBP · Recommended 16:9</small></span></label>{cd.displayImage&&<div className="customer-display-image-preview-v21"><img src={cd.displayImage} alt="Customer display preview"/><span>Preview · Left 50%</span></div>}</div><div className="customer-display-fields"><Field label="COM port"><input value={cd.comPort||""} onChange={e=>patch("customerDisplay","comPort",e.target.value)} placeholder="COM3"/></Field><Field label="Baud rate"><input type="number" min="300" value={cd.baudRate??9600} onChange={e=>patch("customerDisplay","baudRate",Number(e.target.value))}/></Field><Field label="Data bits"><select value={cd.dataBits??8} onChange={e=>patch("customerDisplay","dataBits",Number(e.target.value))}><option value="7">7</option><option value="8">8</option></select></Field><Field label="Stop bits"><select value={cd.stopBits??1} onChange={e=>patch("customerDisplay","stopBits",Number(e.target.value))}><option value="1">1</option><option value="2">2</option></select></Field><Field label="Number of characters"><input type="number" min="8" max="80" value={cd.characters??20} onChange={e=>patch("customerDisplay","characters",Number(e.target.value))}/></Field><Field label="Welcome top line"><input value={cd.topLine||"WELCOME!"} onChange={e=>patch("customerDisplay","topLine",e.target.value)}/></Field><Field label="Welcome bottom line"><input value={cd.bottomLine||""} onChange={e=>patch("customerDisplay","bottomLine",e.target.value)}/></Field></div><div className="customer-display-actions-v21"><button type="button" className="customer-display-test" onClick={testDisplay}>▤ Test customer display</button><span>For second monitor mode, make sure Windows is set to <b>Extend these displays</b>.</span></div></div>}
