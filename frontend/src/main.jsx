...
const printInvoice=(sale,company={},customers=[])=>{
 const customer=(customers||[]).find(x=>x.id===sale?.customerId);
 const items=(sale?.items||[]).map(i=>`<tr><td>${escapeHtml(i.qty)}</td><td>${escapeHtml(i.name)}</td><td class="num">${escapeHtml(money(Number(i.price||0)*Number(i.qty||0)))}</td></tr>`).join("");
 const html=`<div class="head"><h1>${escapeHtml(company?.name||"Shining Pearl Tinted")}</h1><div class="meta">${escapeHtml(company?.phoneNumber||"")} · INVOICE ${escapeHtml(sale?.no||"")}</div></div><div class="meta">Date: ${escapeHtml(new Date(sale?.date||Date.now()).toLocaleString())}<br>Customer: ${escapeHtml(customer?.name||"Walk-in Customer")}<br>Payment: ${escapeHtml(sale?.payment||"Cash")}</div><table><thead><tr><th>Qty</th><th>Description</th><th>Amount</th></tr></thead><tbody>${items}</tbody></table><div class="total"><div><span>Subtotal</span><b>${escapeHtml(money(sale?.subtotal||0))}</b></div><div><span>Discount</span><b>${escapeHtml(money(sale?.discount||0))}</b></div><div><span>Tax</span><b>${escapeHtml(money(sale?.tax||0))}</b></div><div class="grand"><span>Total</span><b>${escapeHtml(money(sale?.total||0))}</b></div></div><div class="footer">Thank you · Shining Pearl Tinted</div>`;
 openPrintDocument("Invoice "+(sale?.no||""),html);
};
...
function App(){
 ...
 const emailReceipt=(sale)=>{const c=customers.find(x=>x.id===sale.customerId);let to=c?.email&&c.email!=="-"?c.email:"";if(!to)to=window.prompt("Email recipient","")||"";if(!to)return;const body=["Dear "+(c?.name||"Customer")+",","","Please find your sales receipt details below.","Receipt: "+sale.no,"Date: "+new Date(sale.date).toLocaleString(),"Total: "+money(sale.total),"Payment: "+sale.payment,"","Thank you,","Shining Pearl Tinted"].join("\n");window.location.href="mailto:"+encodeURIComponent(to)+"?subject="+encodeURIComponent("Receipt "+sale.no)+"&body="+encodeURIComponent(body);};
