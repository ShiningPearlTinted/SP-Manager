...
/* Central MySQL persistence bridge. Existing load/save callers are preserved. */
const SP_DB_API_BASE="https://app.shiningpearltinted.com/SP-Manager-api";
const SP_DB_OUTLET="SP01";
const SP_DB_TIMEOUT_MS=8000;
const SP_DB_EXCLUDED_KEYS=new Set(["auto_backup","auto_backup_close","last_backup_close"]);
let spDbHydrated=false;
let spDbHydratePromise=null;
const spDbSaveTimers=new Map();

const spDbRequest=async(path,options={})=>{
  const controller=new AbortController();
  const timer=setTimeout(()=>controller.abort(),SP_DB_TIMEOUT_MS);
  try{
    const r=await fetch(SP_DB_API_BASE+String(path||""),{cache:"no-store",...options,signal:controller.signal,headers:{"Content-Type":"application/json",...(options.headers||{})}});
    const data=await r.json().catch(()=>null);
    if(!r.ok||data?.ok===false)throw new Error(data?.error||`Database API HTTP ${r.status}`);
    return data||{ok:true};
  }finally{clearTimeout(timer)}
};
const spDbLocalState=()=>{
  const data={};
  for(let i=0;i<localStorage.length;i++){
    const full=localStorage.key(i)||"";if(!full.startsWith("sp_"))continue;
    const key=full.slice(3);if(SP_DB_EXCLUDED_KEYS.has(key)||key.startsWith("auto_backup_"))continue;
    try{data[key]=JSON.parse(localStorage.getItem(full)||"null")}catch{data[key]=localStorage.getItem(full)}
  }return data;
};
const spDbQueueSave=(key,value)=>{
  if(SP_DB_EXCLUDED_KEYS.has(key)||key.startsWith("auto_backup_"))return;
  clearTimeout(spDbSaveTimers.get(key));
  const timer=setTimeout(async()=>{
    try{const u=load("activeUser",null);await spDbRequest("/app-state.php?action=save",{method:"POST",body:JSON.stringify({outlet:SP_DB_OUTLET,key,value,updatedBy:u?.username||u?.name||"SP-Manager"})});}
    catch(error){console.warn("SP-Manager database sync failed:",error?.message||error)}
    finally{spDbSaveTimers.delete(key)}
  },180);
  spDbSaveTimers.set(key,timer);
};
const spDbHydrate=async()=>{
  if(spDbHydratePromise)return spDbHydratePromise;
  spDbHydratePromise=(async()=>{
    try{const local=spDbLocalState();const u=local.activeUser;const response=await spDbRequest("/app-state.php?action=sync",{method:"POST",body:JSON.stringify({outlet:SP_DB_OUTLET,data:local,updatedBy:u?.username||u?.name||"SP-Manager"})});
      const remote=response?.data&&typeof response.data==="object"?response.data:{};Object.entries(remote).forEach(([key,value])=>{if(SP_DB_EXCLUDED_KEYS.has(key)||key.startsWith("auto_backup_"))return;try{localStorage.setItem(`sp_${key}`,JSON.stringify(value))}catch{}});
      spDbHydrated=true;return response;
    }catch(error){console.warn("SP-Manager central database unavailable; using local cache:",error?.message||error);spDbHydrated=true;return null;}
  })();return spDbHydratePromise;
};
const load=(k,d)=>{try{return JSON.parse(localStorage.getItem("sp_"+k))??d}catch{return d}};
const save=(k,v)=>{localStorage.setItem("sp_"+k,JSON.stringify(v));if(spDbHydrated)spDbQueueSave(k,v)};
...
const customerDisplayRequest=async(path,options={})=>{const base=customerDisplayApiBase();if(!base)throw new Error("Customer Display API URL is not configured. Set it in Settings > Customer display.");const cleanPath=String(path||"").replace(/^\/api(?=\/)/,"");const endpoint=base+cleanPath;const r=await fetch(endpoint,{cache:"no-store",...options,headers:{"Content-Type":"application/json",...(options.headers||{})}});let data=null;try{data=await r.json()}catch{}if(!r.ok||data?.ok===false)throw new Error(data?.error||`HTTP ${r.status}`);return data||{ok:true}};
...
function CustomerDisplay(){
 ...
 return <div className="customer-display-screen cd-customer-display-v9">
   ...
   <footer className="customer-display-footer"><span>{state.displayName||displayCode||terminalCode||"Customer Display"}</span><span>{new Date().toLocaleTimeString([], {hour:"2-digit",minute:"2-digit"})}</span></footer>
 </div>;
}

function App(){
 useEffect(()=>{
   if(!spDbHydrated)return;
   const current=spDbLocalState();
   Object.entries(current).forEach(([key,value])=>spDbQueueSave(key,value));
 },[]);
 ...
 useEffect(()=>{if(!customerDisplayTerminal?.enabled||!customerDisplayTerminal?.displayId)return; if(cart.length){publishCustomerDisplay(customerDisplayState);return;} let hold=false;try{hold=Number(sessionStorage.getItem("sp_customer_display_payment_hold")||0)>Date.now()}catch{} if(hold)return; publishCustomerDisplay({state:"IDLE",items:[],subtotal:0,discount:0,tax:0,total:0,paymentAmount:0,change:0,currency:"RM",companyName:company?.name||"Shining Pearl Tinted",displayName:customerDisplayTerminal?.displayName||customerDisplayTerminal?.displayId||"Customer Display"});},[cart,subtotal,disc,tax,grand,customerDisplayTerminal?.enabled,customerDisplayTerminal?.displayId]);
 ...
 const clearCurrentSale=()=>{setCart([]);setDiscount(0);setDiscountFixed(0);publishCustomerDisplay({state:"IDLE",items:[],subtotal:0,discount:0,tax:0,total:0,paymentAmount:0,change:0,currency:"RM",companyName:company?.name||"Shining Pearl Tinted",displayName:customerDisplayTerminal?.displayName||customerDisplayTerminal?.displayId||"Customer Display"});setNotice("Current sale cleared.");};
 ...
 return <div className={"app app-theme-"+String(settings.general.colorScheme||"Light").toLowerCase().replace(/\s+/g,"-")} dir={settings.general.direction||"ltr"} style={{zoom:Number(settings.general.zoom||100)/100}}>
  ...
  {page==="Loyalty"&&<Loyalty customers={customers} setCustomers={setCustomers}/>}
   ...
   {page==="Settings"&&isPermissionAllowed(activeUser,"manageSettings")&&<Settings settings={settings} setSettings={updateSettings} businessDay={businessDay} toggleBusiness={toggleBusiness} taxRate={taxRate} setTaxRate={r=>{setTaxRate(r);save("taxRate",r)}} company={company} customerDisplayTerminal={customerDisplayTerminal} setCustomerDisplayTerminal={v=>{persist("customerDisplayTerminal",v,setCustomerDisplayTerminal)}} onCancel={()=>setPage("POS / Sales")}/>}
...
function Promotions({promos,savePromo,products,categories,productGroups,onRefresh}){
 ...
 const remove=p=>{if(!window.confirm(`Are you sure you want to delete promotion "${p.name}"?`))return;const next=promos.filter(x=>x.id!==p.id);save("promos",next);window.location.reload()};
...
function Settings({settings,setSettings,businessDay,toggleBusiness,taxRate,setTaxRate,company,onCancel,customerDisplayTerminal,setCustomerDisplayTerminal}){
 const [tab,setTab]=useState("General");
 ...
 const openNetworkDisplay=()=>{const base=String(import.meta.env.BASE_URL||"/SP-Manager/").replace(/\/$/,"");const id=encodeURIComponent(customerDisplayTerminal?.displayId||"CD-001");window.open(`${window.location.origin}${base}/customer-display/?display=${id}`,"_blank","noopener,noreferrer")};
 const uploadDisplayImage=e=>{const file=e.target.files?.[0];if(!file)return;if(!file.type.startsWith("image/")){setNotice("Please select an image file.");return}if(file.size>6*1024*1024){setNotice("Customer Display image must be 6 MB or smaller.");return}const reader=new FileReader();reader.onload=async()=>{const imageData=String(reader.result||"");try{const r=await customerDisplayRequest("/customer-display.php?action=image",{method:"POST",body:JSON.stringify({display_code:customerDisplayTerminal?.displayId||"CD-001",image_data:imageData})});setNotice(r?.ok?"Customer Display image saved to MySQL successfully.":"Unable to save Customer Display image.")}catch(err){setNotice("Image upload failed: "+String(err?.message||err))}};reader.readAsDataURL(file)};
 const saveTerminalPairing=async()=>{try{const t={...CUSTOMER_DISPLAY_DEFAULTS,...(customerDisplayTerminal||{})};save("customerDisplayTerminal",t);setCustomerDisplayTerminal(t);await customerDisplayRequest("/customer-display.php?action=terminal",{method:"POST",body:JSON.stringify(t)});setNotice("Terminal and Customer Display pairing saved to MySQL.")}catch(e){setNotice("Pairing save failed: "+String(e?.message||e))}};
 ...
 return <section className="settings-page"><div className="settings-shell">
  ...
  <div className="settings-main">
   ...
   {tab==="Customer display"&&<><h2>Customer Display — Network</h2><p className="settings-help">Pair this POS with one Customer Display. The display reads only the transaction state for this terminal.</p><div className="settings-grid"><Field label="API Base URL" wide><input value={customerDisplayTerminal?.apiBase||""} onChange={e=>setCustomerDisplayTerminal(v=>{const n={...v,apiBase:e.target.value};save("customerDisplayTerminal",n);return n})} placeholder="https://your-hostinger-domain.com"/></Field><Field label="Terminal ID"><input value={customerDisplayTerminal?.terminalId||"POS-PC-01"} onChange={e=>setCustomerDisplayTerminal(v=>{const n={...v,terminalId:e.target.value};save("customerDisplayTerminal",n);return n})}/></Field><Field label="Terminal Name"><input value={customerDisplayTerminal?.terminalName||"POS PC 01"} onChange={e=>setCustomerDisplayTerminal(v=>{const n={...v,terminalName:e.target.value};save("customerDisplayTerminal",n);return n})}/></Field><Field label="Terminal Type"><select value={customerDisplayTerminal?.terminalType||"PC POS"} onChange={e=>setCustomerDisplayTerminal(v=>{const n={...v,terminalType:e.target.value};save("customerDisplayTerminal",n);return n})}><option>PC POS</option><option>Mobile POS</option></select></Field><Field label="Outlet"><input value={customerDisplayTerminal?.outletId||"SP01"} onChange={e=>setCustomerDisplayTerminal(v=>{const n={...v,outletId:e.target.value};save("customerDisplayTerminal",n);return n})}/></Field><Field label="Customer Display ID"><input value={customerDisplayTerminal?.displayId||"CD-001"} onChange={e=>setCustomerDisplayTerminal(v=>{const n={...v,displayId:e.target.value};save("customerDisplayTerminal",n);return n})}/></Field><Field label="Customer Display Name"><input value={customerDisplayTerminal?.displayName||"Customer Display 001"} onChange={e=>setCustomerDisplayTerminal(v=>{const n={...v,displayName:e.target.value};save("customerDisplayTerminal",n);return n})}/></Field><Field label="Enabled"><Toggle checked={customerDisplayTerminal?.enabled!==false} onChange={v=>setCustomerDisplayTerminal(x=>{const n={...x,enabled:v};save("customerDisplayTerminal",n);return n})}/></Field></div><div className="customer-display-settings-media"><div><b>Promotion / Idle Image</b><p className="settings-help">Upload a PNG, JPG or WebP image. It is stored in the Customer Display MySQL record and shown on the left 50% of the TV.</p></div><label className="logo-add logo-add-ar">＋ Upload display image<input type="file" accept="image/png,image/jpeg,image/webp" onChange={uploadDisplayImage}/></label></div><div className="settings-database-actions"><button type="button" onClick={saveTerminalPairing}>✓ Save terminal pairing</button><button type="button" onClick={openNetworkDisplay}>Open Customer Display</button><button type="button" onClick={()=>setNotice("Customer Display URL: "+window.location.origin+(String(import.meta.env.BASE_URL||"/SP-Manager/").replace(/\/$/,""))+"/customer-display/?display="+encodeURIComponent(customerDisplayTerminal?.displayId||"CD-001"))}>Show URL</button></div><hr/><h2>Native Customer Display</h2><div className="settings-grid"><Field label="Enabled"><Toggle checked={cd.enabled} onChange={v=>patch("customerDisplay","enabled",v)}/></Field><Field label="Secondary monitor"><Toggle checked={cd.secondaryMonitor} onChange={v=>patch("customerDisplay","secondaryMonitor",v)}/></Field><Field label="COM port"><input value={cd.comPort||""} onChange={e=>patch("customerDisplay","comPort",e.target.value)} placeholder="COM3"/></Field><Field label="Baud rate"><input type="number" min="300" value={cd.baudRate??9600} onChange={e=>patch("customerDisplay","baudRate",Number(e.target.value))}/></Field><Field label="Data bits"><select value={cd.dataBits??8} onChange={e=>patch("customerDisplay","dataBits",Number(e.target.value))}><option value="7">7</option><option value="8">8</option></select></Field><Field label="Stop bits"><select value={cd.stopBits??1} onChange={e=>patch("customerDisplay","stopBits",Number(e.target.value))}><option value="1">1</option><option value="2">2</option></select></Field><Field label="Number of characters"><input type="number" min="8" max="80" value={cd.characters??20} onChange={e=>patch("customerDisplay","characters",Number(e.target.value))}/></Field><Field label="Welcome top line"><input value={cd.topLine||"WELCOME!"} onChange={e=>patch("customerDisplay","topLine",e.target.value)}/></Field><Field label="Welcome bottom line"><input value={cd.bottomLine||""} onChange={e=>patch("customerDisplay","bottomLine",e.target.value)}/></Field></div><button onClick={testDisplay}>▤ Test customer display</button><p className="settings-help">SP-Manager's native customer display is serial/COM based; a normal second monitor is not the same feature. SP-Manager keeps the secondary-monitor option separate so it does not falsely claim native SP-Manager parity.</p></>}
...
function CentralDatabaseBootstrap({children}){
 const[ready,setReady]=useState(false);
 useEffect(()=>{let alive=true;const fallback=setTimeout(()=>{if(alive)setReady(true)},3500);spDbHydrate().finally(()=>{clearTimeout(fallback);if(alive)setReady(true)});return()=>{alive=false;clearTimeout(fallback)}},[]);
 if(!ready)return <div style={{minHeight:"100vh",display:"grid",placeItems:"center",background:"#0b1119",color:"#f8fafc",fontFamily:"Segoe UI,Arial,sans-serif"}}><div style={{textAlign:"center",padding:24}}><div style={{fontSize:26,fontWeight:800}}>SP-Manager</div><div style={{marginTop:8,color:"#94a3b8"}}>Connecting to central database…</div></div></div>;
 return children;
}
...
createRoot(document.getElementById("root")).render(<AppErrorBoundary><CentralDatabaseBootstrap><App/></CentralDatabaseBootstrap></AppErrorBoundary>);
