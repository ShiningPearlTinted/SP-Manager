...
function App(){
 ...
 const add=(p,quantityOverride)=>{if(p?.active===false){setNotice("This product is inactive and cannot be sold.");return}if(!settings.products.allowNegativePrice&&Number(p.price||0)<0){setNotice("Negative price is not allowed for "+p.name+".");return}const service=Boolean(p.isService||p.service||p.serviceItem);if(settings.order.preventSaleBelowCost&&!service&&Number(p.price||0)<Number(p.cost||0)){setNotice("Sale below cost price is prevented for "+p.name+".");return}const existing=settings.order.separateRow?null:cart.find(x=>x.id===p.id);const productDefaultQuantity=p.defaultQuantity===false?1:Math.max(1,Number(settings.order.defaultQuantity||1));const step=Math.max(1,Number(quantityOverride||productDefaultQuantity));const nextQty=(existing?.qty||0)+step;if(settings.order.preventNegativeInventory&&!service&&nextQty>Number(p.stock||0)){setNotice("Insufficient stock for "+p.name+".");return}setCart(c=>{const old=settings.order.separateRow?null:c.find(x=>x.id===p.id);const qty=(old?.qty||0)+step;const price=old?.manualPrice?old.price:promotionPrice(p,qty);const line=old?{...old,qty,price,isService:service}:{...p,id:p.id,productId:p.id,lineId:settings.order.separateRow?uid():p.id,qty,originalPrice:p.price,price,isService:service,manualPrice:false};return old?c.map(x=>x.id===p.id?line:x):[...c,line]});};
...
function POS({updateLinePrice,filtered,q,setQ,posSearchMode,setPosSearchMode,add,cart,changeQty,customers,setCustomers,customer,setCustomer,discount,setDiscount,discountFixed,setDiscountFixed,payment,setPayment,paymentTypes,subtotal,disc,taxRate,setTaxRate,tax,grand,sale,saveOpenOrder,orders,setOrders,updateSaleNote,setNoteBox,clearCurrentSale,printReceipt,closeLastSale,categories,posCategory,setPosCategory,products,company,lastSale,menuOpen,setMenuOpen,setPage,sales,emailReceipt,settings,activeUser,signOut,openCashInOut,openCashDrawer,openEndOfDayFromPOS}){
 ...
 const addPos=p=>{if(saleLocked){setNoticeLocal("Sale is locked. Unlock the sale before changing items.");return}add(p,nextQuantity);setNextQuantity(1);setSelectedLineId(p?.id??null);if(settings.order.sounds&&settings.order.soundItemAdded)playPosBeep("ok");if(p&&priceChangeAllowedFor(p)){setPriceValue(String(Number(p.price||0)));setPriceScreen(true)}};
 ...
 useEffect(()=>{const onKey=e=>{
   ...
   if(e.key==="Escape"){setPaymentScreen(false);setSplitScreen(false);setDiscountScreen(false);setTransferScreen(false);setSavedScreen(false);setQuantityScreen(false);setPriceScreen(false);setSelectedLineId(null);document.querySelector('.ar-search-input')?.focus()}
 ...
 const openPrice=()=>{if(saleLocked){setNoticeLocal("Sale is locked. Unlock the sale before changing price.");return}const i=cart.find(x=>(x.lineId||x.id)===selectedLineId||x.productId===selectedLineId);if(!i){setNoticeLocal("Select an item before changing price.");return}const product=products.find(p=>String(p.id)===String(i.productId??i.id))||i;if(!priceChangeAllowedFor(product)){setNoticeLocal("Price change is not allowed for this product.");return}setPriceValue(String(Number(i.price||0)));setPriceScreen(true)};
 const applyPrice=()=>{const i=cart.find(x=>(x.lineId||x.id)===selectedLineId||x.productId===selectedLineId);if(!i){setPriceScreen(false);return}const product=products.find(p=>String(p.id)===String(i.productId??i.id))||i;if(!priceChangeAllowedFor(product)){setNoticeLocal("Price change is not allowed for this product.");setPriceScreen(false);return}if(updateLinePrice(i.lineId||i.id,priceValue))setPriceScreen(false)};
 ...
 return <section className={"ar-pos-shell "+(posFullscreen?"pos-browser-fullscreen":"")}>
  ...
  <div className="ar-main">
   <div className="ar-order-panel">
    ...
    <div className="ar-order-items">{cart.map(i=><div className={"ar-order-item "+((i.lineId||i.id)===selectedLineId?"selected":"")} key={i.lineId||i.id} onClick={()=>{const key=i.lineId||i.id;setSelectedLineId(key);const product=products.find(p=>String(p.id)===String(i.productId??i.id))||i;if(!saleLocked&&priceChangeAllowedFor(product)){setPriceValue(String(Number(i.price||0)));setPriceScreen(true)}}}><button className="ar-item-plus" onClick={(e)=>{e.stopPropagation();addPos(i)}}>＋</button><div><b>{i.name}</b><small>{i.code||'—'} · {money(i.price)} × {i.qty}</small></div><strong>{money(i.price*i.qty)}</strong></div>)}{!cart.length&&<div className="ar-no-items"><span>🛒</span><b>No items</b><small>Select a category or search for a product</small></div>}</div>
...
<div className="ar-user-menu-footer spmanager-footer-controls">
 ...
 <button type="button" title="Exit application" aria-label="Exit application" onClick={exitPosApplication}><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 3v9M6.2 5.8a8 8 0 1 0 11.6 0"/></svg></button>
  ...
  {priceScreen&&<div className="ar-modal-backdrop price-keypad-backdrop" onMouseDown={()=>setPriceScreen(false)}><div className="ar-modal price-keypad-modal" onMouseDown={e=>e.stopPropagation()}><div className="price-keypad-head"><div><small>POS / PRICE</small><h3>Change price</h3>{(()=>{const i=cart.find(x=>(x.lineId||x.id)===selectedLineId||x.productId===selectedLineId);return i?<p>Product <b>"{i.name}"</b><br/><span>Original price: {money(i.originalPrice??i.price)}</span></p>:null})()}</div><button type="button" aria-label="Close" onClick={()=>setPriceScreen(false)}>×</button></div><div className="price-keypad-display"><input autoFocus aria-label="New price" type="text" inputMode="decimal" value={priceValue} onChange={e=>setPriceValue(e.target.value.replace(/[^0-9.\-]/g,""))} onKeyDown={e=>{if(e.key==="Enter")applyPrice();if(e.key==="Escape")setPriceScreen(false)}}/><span>RM</span></div><div className="price-keypad-grid">{['1','2','3','⌫','4','5','6','Esc','7','8','9','Enter','−','0','.'].map((k,i)=><button key={i} type="button" className={k==='Enter'?'enter-key':k==='Esc'?'esc-key':k==='⌫'?'backspace-key':k==='−'?'minus-key':''} onClick={()=>{if(!k)return;if(k==='Enter'){applyPrice();return}if(k==='Esc'){setPriceScreen(false);return}if(k==='⌫'){setPriceValue(v=>String(v).slice(0,-1));return}if(k==='−'){setPriceValue(v=>v.startsWith('-')?v.slice(1):'-'+v);return}setPriceValue(v=>String(v||'')+k)}}>{k}</button>)}</div><div className="price-keypad-footer"><button type="button" className="secondary" onClick={()=>setPriceScreen(false)}>Cancel</button><button type="button" onClick={applyPrice}>Apply price</button></div></div></div>}
