 ...
 return <div className="customer-display-screen cd-modern-v4">
   ...
   <main className="customer-display-main">
    ...
    <section className="customer-display-order">
      ...
      <div className="customer-display-items">{(state.items||[]).map((item,i)=><div className="customer-display-item" key={item.lineId||item.id||i}><div><b>{item.name||"Product"}</b><small>{item.code||""}</small></div><strong className="cd-item-qty">{Number(item.qty||0)}</strong><strong className="cd-item-amount">{fmt(Number(item.price||0)*Number(item.qty||0))}</strong></div>)}</div>
