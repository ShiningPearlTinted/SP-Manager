...
const buildInvoiceHtml=(sale,company={},customers=[],settings={})=>{
 ...
 const itemRows=items.map((i,n)=>`<tr><td class="col-no">${n+1}</td><td class="col-item">${escapeHtml(i.name||"")}</td><td class="col-qty num">${escapeHtml(i.qty||0)}</td><td class="col-unit num">${escapeHtml(Number(i.price||0).toFixed(2))}</td>${showTax?`<td class="col-tax num">---</td>`:""}${showDiscount?`<td class="col-discount num">0.00%</td>`:""}<td class="col-total num">${escapeHtml(money(Number(i.price||0)*Number(i.qty||0)))}</td></tr>`).join("");
