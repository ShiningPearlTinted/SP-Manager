...
function Settings({settings,setSettings,businessDay,toggleBusiness,taxRate,setTaxRate,company,onCancel}){
 ...
 const testDisplay=()=>hardwareRequestLocal(hw.agentUrl,"/display",{port:cd.comPort||hw.customerDisplayPort,baud:Number(cd.baudRate||hw.customerDisplayBaud||9600),dataBits:Number(cd.dataBits||8),stopBits:Number(cd.stopBits||1),line1:cd.topLine||hw.customerDisplayTop||"WELCOME!",line2:cd.bottomLine||hw.customerDisplayBottom||"",chars:Number(cd.characters||20),mode:cd.secondaryMonitor?"secondary":String(hw.customerDisplayMode||"COM").toLowerCase()}).then(()=>cd.secondaryMonitor?hardwareRequestLocal(hw.agentUrl,"/display-window",{}):null).then(()=>setNotice("Customer display test sent successfully.")).catch(e=>setNotice("Customer display test failed: "+e.message));
