...
function App(){
 ...
 useEffect(()=>{
  let alive=true;
  const key="sp_central_state_bootstrap_v1";
  const run=async()=>{
   try{
    const r=await centralRequest("all",{outlet_id:"SP01"});
    if(!alive)return;
    const remote=r?.data||{};
    const keys=Object.keys(remote).filter(k=>!CENTRAL_STATE_EXCLUDE.has(k));
    if(keys.length){
      keys.forEach(k=>{try{localStorage.setItem("sp_"+k,JSON.stringify(remote[k]))}catch{}});
      if(sessionStorage.getItem(key)!=="1"){sessionStorage.setItem(key,"1");window.location.reload();return;}
    }else{
      const states={};
      for(let i=0;i<localStorage.length;i++){const k=localStorage.key(i);if(!k||!k.startsWith("sp_")||k.startsWith("sp_auto_backup_")||k.startsWith("sp_"+key))continue;const name=k.slice(3);if(CENTRAL_STATE_EXCLUDE.has(name))continue;try{states[name]=JSON.parse(localStorage.getItem(k))}catch{}}
      if(Object.keys(states).length)await centralRequest("save-batch",{outlet_id:"SP01",states,updated_by:"SP-Manager"});
      sessionStorage.setItem(key,"1");
    }
    try{const pr=await centralProductsRequest("list",{outlet_id:"SP01"});if(Number(pr?.count||0)>0){const mapped=(pr.products||[]).map(centralProductToApp).filter(Boolean);if(mapped.length){localStorage.setItem("sp_products",JSON.stringify(mapped));}}}catch(e){console.warn("Central products sync unavailable; local products remain active.",e?.message||e)}
...
function Products({products,setProducts,addProduct,updateProduct,editing,setEditing,categories,setCategories,productGroups,setProductGroups,groupMeta,setGroupMeta,suppliers=[],setNotice,settings=defaultSettings}){
 ...
 const confirmDeleteProduct=async()=>{
  ...
  try{await centralProductsRequest("delete",{outlet_id:"SP01",id:target.id});const next=products.filter(p=>p.id!==target.id);save("products",next);setProducts(next);if(editing?.id===target.id){setEditing(null);setShowEditor(false)}setSelectedProductId(null);setProductDeleteTarget(null);setNotice("Product deleted successfully from SP-Central.");}
  catch(e){setNotice("Product was not deleted from MySQL: "+(e?.message||"Unknown error"));}
 };
