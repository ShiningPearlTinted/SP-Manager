...
<div className="ar-user-menu-footer spmanager-footer-controls">
 ...
 <button type="button" title="Exit application" aria-label="Exit application" onClick={exitPosApplication}><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 3v9M6.2 5.8a8 8 0 1 0 11.6 0"/></svg></button>
  ...
  {priceScreen&&<div className="ar-modal-backdrop quantity-modal-backdrop" onMouseDown={()=>setPriceScreen(false)}><div className="ar-modal quantity-modal price-change-modal" onMouseDown={e=>e.stopPropagation()}><div className="ar-modal-head quantity-modal-head"><div><h3>Change price</h3>{(()=>{const i=cart.find(x=>(x.lineId||x.id)===selectedLineId||x.productId===selectedLineId);return i?<small>Product "{i.name}"<br/>Original price: {money(i.originalPrice??i.price)}</small>:null})()}</div><button type="button" onClick={()=>setPriceScreen(false)}>×</button></div><div className="quantity-modal-body"><label>Price (RM)<input autoFocus type="number" min="0" step="0.01" value={priceValue} onChange={e=>setPriceValue(e.target.value)} onKeyDown={e=>{if(e.key==="Enter")applyPrice();if(e.key==="Escape")setPriceScreen(false)}}/></label></div><div className="modal-actions quantity-modal-actions"><button type="button" className="secondary" onClick={()=>setPriceScreen(false)}>Cancel</button><button type="button" onClick={applyPrice}>OK</button></div></div></div>}
