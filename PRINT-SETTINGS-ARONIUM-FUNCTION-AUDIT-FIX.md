# SP-Manager Print Settings — Aronium Function Audit & Fix

Base: SP-Manager 1.0.18 PRINT SETTINGS A5 FUNCTION FIX
Reference: Aronium(8).zip and Aronium Help Center Print options documentation.

## Findings

The existing Print > Printer selection screen had the printer-operation cog button, but the cog only changed the selected operation. It did not open a dedicated printer-settings dialog like the Aronium workflow.
...
- Advanced exposes code page and character set for Generic / Text only; other printer types show the Aronium-style unavailable notice.
