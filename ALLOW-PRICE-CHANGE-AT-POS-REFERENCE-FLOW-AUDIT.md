# Allow Price Change at POS — SP-Manager reference Flow Fix

## Source basis
Reviewed the supplied SP-Manager reference source archive and SP-Manager reference Help Center. SP-Manager reference documents that `Price change allowed` lets the user change/confirm the price **before every sale**. The implementation therefore opens the modern SP-Manager Change Price keypad before the product is committed to the cart.
