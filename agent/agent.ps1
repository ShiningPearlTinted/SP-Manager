...
<style>
:root{--gold:#d6ad55;--gold2:#f1d78b;--bg:#07090d;--panel:rgba(255,255,255,.045);--muted:rgba(255,255,255,.62)}
*{box-sizing:border-box}html,body{margin:0;width:100%;height:100%;background:radial-gradient(circle at 50% -10%,#1a1d25 0,#090b10 42%,#050609 100%);color:#fff;font-family:Segoe UI,Arial,sans-serif;overflow:hidden}
body{display:flex;align-items:center;justify-content:center;padding:clamp(24px,4vw,70px)}
.shell{width:min(1500px,100%);height:min(850px,100%);display:flex;flex-direction:column;position:relative}
.top{display:flex;align-items:center;justify-content:space-between;gap:30px;padding:0 0 28px;border-bottom:1px solid rgba(255,255,255,.12)}
.brand-wrap{display:flex;align-items:center;gap:22px;min-width:0}.logo{width:76px;height:76px;border-radius:18px;object-fit:contain;background:rgba(255,255,255,.06);padding:10px;display:none}.logo.show{display:block}.fallback{width:76px;height:76px;border-radius:18px;display:flex;align-items:center;justify-content:center;background:linear-gradient(135deg,#161a22,#0b0d12);border:1px solid rgba(214,173,85,.45);color:var(--gold2);font-size:28px;font-weight:800;letter-spacing:-.04em}.brand-name{font-size:clamp(18px,2vw,30px);font-weight:800;letter-spacing:.04em;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.brand-sub{margin-top:5px;font-size:12px;letter-spacing:.2em;text-transform:uppercase;color:var(--muted)}
.status{font-size:12px;letter-spacing:.12em;text-transform:uppercase;color:var(--muted);white-space:nowrap}.main{flex:1;display:grid;grid-template-columns:minmax(0,1fr) minmax(320px,40%);gap:clamp(25px,5vw,80px);align-items:center;padding:clamp(35px,6vh,90px) 0}.welcome{color:var(--muted);font-size:clamp(16px,1.5vw,25px);letter-spacing:.08em;text-transform:uppercase;margin-bottom:18px}.item{font-size:clamp(38px,5vw,86px);font-weight:750;line-height:1.05;white-space:normal;overflow:hidden;display:-webkit-box;-webkit-line-clamp:3;-webkit-box-orient:vertical}.summary{background:var(--panel);border:1px solid rgba(255,255,255,.10);border-radius:28px;padding:clamp(28px,4vw,55px);text-align:right;box-shadow:0 24px 70px rgba(0,0,0,.28)}.summary-label{font-size:13px;letter-spacing:.18em;text-transform:uppercase;color:var(--muted);margin-bottom:14px}.total{font-size:clamp(48px,6vw,105px);font-weight:900;line-height:1;white-space:nowrap}.currency{font-size:.46em;vertical-align:middle;margin-right:.08em;color:var(--gold2)}.bottom{display:flex;justify-content:space-between;gap:20px;border-top:1px solid rgba(255,255,255,.12);padding-top:20px;color:var(--muted);font-size:12px}.accent{color:var(--gold2)}
@media(max-width:800px){.main{grid-template-columns:1fr}.summary{text-align:left}.top{padding-bottom:18px}.logo,.fallback{width:58px;height:58px}.brand-sub{display:none}.status{display:none}}
</style></head>
<body><div class="shell"><div class="top"><div class="brand-wrap"><img class="logo" id="logo" alt="Company logo"><div class="fallback" id="fallback">SP</div><div><div class="brand-name" id="brand">SHINING PEARL TINTED</div><div class="brand-sub">Customer Display</div></div></div><div class="status">Thank you for shopping with us</div></div><div class="main"><div><div class="welcome" id="welcome">WELCOME!</div><div class="item" id="item">Your transaction will appear here</div></div><div class="summary"><div class="summary-label">Total</div><div class="total" id="total"><span class="currency">RM</span>0.00</div></div></div><div class="bottom"><span id="footerLeft">Premium automotive care</span><span class="accent" id="footerRight">Thank you</span></div></div>
<script>
function moneyText(v){const n=Number(v);return Number.isFinite(n)?n.toLocaleString('en-MY',{minimumFractionDigits:2,maximumFractionDigits:2}):'0.00'}
function render(d){const logo=document.getElementById('logo'),fallback=document.getElementById('fallback');if(d.logoData){logo.src=d.logoData;logo.classList.add('show');fallback.style.display='none'}else{logo.classList.remove('show');fallback.style.display='flex'}document.getElementById('brand').textContent=d.brandName||'SHINING PEARL TINTED';document.getElementById('welcome').textContent=d.line1||'WELCOME!';document.getElementById('item').textContent=d.itemName||d.message||'Your transaction will appear here';document.getElementById('total').innerHTML='<span class="currency">RM</span>'+moneyText(d.total||0);document.getElementById('footerLeft').textContent=d.footerLeft||'Premium automotive care';document.getElementById('footerRight').textContent=d.footerRight||d.line2||'Thank you'}
async function tick(){try{const r=await fetch('/display-state',{cache:'no-store'});render(await r.json())}catch(e){}}tick();setInterval(tick,400)
</script></body></html>
'@
...
'@
      ...
      $chars=[Math]::Max(8,[int]$b.chars);$line1=([string]$b.line1).PadRight($chars).Substring(0,$chars);$line2=([string]$b.line2).PadRight($chars).Substring(0,$chars);$script:DisplayState=@{line1=$line1;line2=$line2;itemName=([string]$b.itemName);message=([string]$b.message);total=[double]($b.total);brandName=([string]$b.brandName);logoData=([string]$b.logoData);footerLeft=([string]$b.footerLeft);footerRight=([string]$b.footerRight);chars=$chars;updatedAt=(Get-Date).ToUniversalTime().ToString('o')}
