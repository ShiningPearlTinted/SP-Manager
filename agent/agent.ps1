...
$script:DisplayState=@{line1='WELCOME!';line2='';chars=20;displayImage='';updatedAt=$StartedAt}
$script:DisplayWindowHandle=[IntPtr]::Zero
...
:root{--gold:#d6b36a;--gold2:#f4d58c;--red:#c1121f;--bg:#07090d;--panel:rgba(255,255,255,.055);--line:rgba(255,255,255,.1);--muted:#9aa3b2}*{box-sizing:border-box}html,body{margin:0;width:100%;height:100%;overflow:hidden;background:radial-gradient(circle at 15% 10%,rgba(193,18,31,.18),transparent 30%),radial-gradient(circle at 85% 90%,rgba(214,179,106,.12),transparent 32%),var(--bg);color:#fff;font-family:Segoe UI,Arial,sans-serif}.shell{width:100%;height:100%;display:grid;grid-template-columns:50% 50%;gap:0;overflow:hidden}.media{height:100%;display:flex;align-items:center;justify-content:center;background:#05070a;border-right:1px solid var(--line);overflow:hidden}.media img{width:100%;height:100%;object-fit:cover;display:none}.media-empty{padding:30px;text-align:center;color:var(--muted)}.media-empty b{display:block;color:var(--gold2);font-size:clamp(20px,2vw,34px);margin-bottom:10px}.content{height:100%;min-width:0;display:flex;flex-direction:column;padding:clamp(22px,3vw,55px);overflow:hidden}.top{display:flex;align-items:center;gap:18px;padding:0 0 22px;border-bottom:1px solid var(--line);flex:0 0 auto}.logo{width:58px;height:58px;border-radius:15px;object-fit:contain;background:#fff;padding:6px;display:none;box-shadow:0 10px 30px rgba(0,0,0,.35)}.logo.show{display:block}.fallback{width:58px;height:58px;border-radius:15px;background:linear-gradient(145deg,#171b25,#090b10);border:1px solid rgba(214,179,106,.5);display:flex;align-items:center;justify-content:center;font-size:23px;font-weight:900;color:var(--gold2);letter-spacing:-2px}.brand{font-size:clamp(14px,1.4vw,22px);font-weight:800;letter-spacing:.16em;text-transform:uppercase}.sub{margin-top:5px;color:var(--muted);font-size:clamp(10px,1vw,14px);letter-spacing:.08em}.main{flex:1;min-height:0;display:flex;align-items:center;justify-content:center;overflow:hidden}.welcome{text-align:center;width:100%}.welcome .eyebrow{color:var(--gold2);font-size:clamp(11px,1.2vw,17px);font-weight:700;letter-spacing:.22em;text-transform:uppercase}.welcome h1{font-size:clamp(42px,5vw,92px);line-height:.95;margin:18px 0 10px;font-weight:900;letter-spacing:-.04em}.welcome p{margin:0;color:var(--muted);font-size:clamp(13px,1.4vw,21px)}.sale{width:100%;max-width:100%;background:linear-gradient(145deg,rgba(255,255,255,.075),rgba(255,255,255,.025));border:1px solid var(--line);border-radius:24px;padding:clamp(20px,2.6vw,38px);box-shadow:0 25px 70px rgba(0,0,0,.35);backdrop-filter:blur(16px);overflow:hidden}.sale-head{display:flex;justify-content:space-between;align-items:flex-start;gap:20px;margin-bottom:18px}.sale-title{font-size:clamp(23px,2.6vw,44px);font-weight:850;line-height:1.05}.sale-label{color:var(--gold2);font-size:clamp(10px,1vw,15px);font-weight:700;letter-spacing:.15em;text-transform:uppercase;margin-bottom:7px}.items{display:flex;flex-direction:column;gap:6px;max-height:42vh;overflow:hidden}.item{display:grid;grid-template-columns:1fr auto auto;gap:14px;align-items:center;padding:12px 0;border-bottom:1px solid rgba(255,255,255,.08)}.item-name{font-size:clamp(15px,1.5vw,24px);font-weight:650}.qty{color:var(--muted);font-size:clamp(12px,1.1vw,18px)}.price{font-size:clamp(15px,1.5vw,24px);font-weight:750}.total-row{display:flex;align-items:flex-end;justify-content:space-between;gap:20px;margin-top:20px;padding-top:18px;border-top:1px solid rgba(214,179,106,.35)}.total-label{color:var(--muted);font-size:clamp(11px,1vw,17px);text-transform:uppercase;letter-spacing:.14em}.total{font-size:clamp(34px,4.5vw,70px);line-height:.9;font-weight:950;letter-spacing:-.045em;color:#fff}.footer{display:flex;justify-content:space-between;color:#697386;font-size:10px;letter-spacing:.08em;text-transform:uppercase;padding-top:15px;flex:0 0 auto}.accent{color:var(--gold2)}
@media(max-width:900px){.shell{grid-template-columns:50% 50%}.content{padding:18px}.top{padding-bottom:14px}.logo,.fallback{width:44px;height:44px}.fallback{font-size:18px}.sale{border-radius:16px;padding:16px}.item{grid-template-columns:1fr auto;gap:8px}.price{grid-column:2}.qty{grid-column:1}.total-row{flex-direction:column;align-items:flex-start}.total{font-size:48px}}
</style></head><body><div class="shell"><section class="media"><img id="displayImage" alt="Customer display image"><div id="mediaEmpty" class="media-empty"><b>SHINING PEARL TINTED</b><span>Customer display</span></div></section><section class="content"><header class="top"><img id="logo" class="logo" alt="Company logo"><div id="fallback" class="fallback">SP</div><div><div id="brand" class="brand">SHINING PEARL TINTED</div><div class="sub">Customer Display</div></div></header><main class="main"><section id="welcome" class="welcome"><div class="eyebrow">Welcome</div><h1 id="welcomeTitle">WELCOME!</h1><p id="welcomeText">Thank you for visiting <span class="accent">Shining Pearl Tinted</span></p></section><section id="sale" class="sale" style="display:none"><div class="sale-head"><div><div class="sale-label">Your order</div><div id="saleTitle" class="sale-title">Thank you for your purchase</div></div></div><div id="items" class="items"></div><div class="total-row"><div class="total-label">Total</div><div id="total" class="total">RM 0.00</div></div></section></main><footer class="footer"><span>SP-MANAGER</span><span id="status">Ready</span></footer></section></div><script>
const $=id=>document.getElementById(id);function esc(v){return String(v??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]))}function money(v,c='RM'){return c+' '+Number(v||0).toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})}function render(d){const logo=$('logo'),fb=$('fallback');if(d.logo){logo.src=d.logo;logo.classList.add('show');fb.style.display='none'}else{logo.classList.remove('show');fb.style.display='flex'}const image=$('displayImage'),empty=$('mediaEmpty');if(d.displayImage){image.src=d.displayImage;image.style.display='block';empty.style.display='none'}else{image.removeAttribute('src');image.style.display='none';empty.style.display='block'}$('brand').textContent=d.companyName||'SHINING PEARL TINTED';const items=Array.isArray(d.items)?d.items:[];if(items.length){$('welcome').style.display='none';$('sale').style.display='block';$('saleTitle').textContent=d.customerName?('Thank you, '+d.customerName):'Your order';$('items').innerHTML=items.map(i=>`<div class="item"><div class="item-name">${esc(i.name||'Item')}</div><div class="qty">× ${esc(i.qty||1)}</div><div class="price">${money((Number(i.price)||0)*(Number(i.qty)||1),d.currency||'RM')}</div></div>`).join('');$('total').textContent=money(d.total,d.currency||'RM')}else{$('sale').style.display='none';$('welcome').style.display='block';$('welcomeTitle').textContent=d.line1||'WELCOME!';$('welcomeText').innerHTML='Thank you for visiting <span class="accent">'+esc(d.companyName||'Shining Pearl Tinted')+'</span>'}$('status').textContent='Ready'}async function tick(){try{const r=await fetch('/display-state',{cache:'no-store'});const d=await r.json();render(d)}catch(e){}}tick();setInterval(tick,400)</script></body></html>
'@
...
'@
      ...
      if($script:DisplayWindowHandle -ne [IntPtr]::Zero -and [SPDisplayWindow]::IsWindow([IntPtr]$script:DisplayWindowHandle)){
        [SPDisplayWindow]::ShowWindowAsync([IntPtr]$script:DisplayWindowHandle,3)|Out-Null
        [SPDisplayWindow]::SetWindowPos([IntPtr]$script:DisplayWindowHandle,[IntPtr]::Zero,$screen.Bounds.X,$screen.Bounds.Y,$screen.Bounds.Width,$screen.Bounds.Height,0x0040)|Out-Null
        Send-Json $s 200 @{ok=$true;mode='secondary';details=@{monitor=$screen.DeviceName;x=$screen.Bounds.X;y=$screen.Bounds.Y;width=$screen.Bounds.Width;height=$screen.Bounds.Height;windowFound=$true;reused=$true}};return
      }
      $browser=$null
      ...
      $script:DisplayWindowHandle=[IntPtr]$hwnd
    ...
    if($req.method -eq 'POST' -and $req.path -eq '/display-state'){$chars=[Math]::Max(8,[int]$b.chars);$line1=([string]$b.line1).PadRight($chars).Substring(0,$chars);$line2=([string]$b.line2).PadRight($chars).Substring(0,$chars);$script:DisplayState=@{line1=$line1;line2=$line2;chars=$chars;companyName=[string]$b.companyName;logo=[string]$b.logo;displayImage=[string]$b.displayImage;items=@($b.items);total=[double]$b.total;currency=([string]$b.currency);customerName=[string]$b.customerName;updatedAt=(Get-Date).ToUniversalTime().ToString('o')};Send-Json $s 200 @{ok=$true};return}
    if($req.method -eq 'POST' -and $req.path -eq '/display'){$chars=[Math]::Max(8,[int]$b.chars);$line1=([string]$b.line1).PadRight($chars).Substring(0,$chars);$line2=([string]$b.line2).PadRight($chars).Substring(0,$chars);$script:DisplayState=@{line1=$line1;line2=$line2;chars=$chars;companyName=[string]$b.companyName;logo=[string]$b.logo;displayImage=[string]$b.displayImage;items=@($b.items);total=[double]$b.total;currency=([string]$b.currency);customerName=[string]$b.customerName;updatedAt=(Get-Date).ToUniversalTime().ToString('o')};$portName=[string]$b.port;if(!$portName){throw 'COM port is required'};$chars=[Math]::Max(8,[int]$b.chars);$baud=[int]$b.baud;if(!$baud){$baud=9600};$sp=New-Object IO.Ports.SerialPort($portName,$baud,'None',8,'One');$sp.Open();$line1=([string]$b.line1).PadRight($chars).Substring(0,$chars);$line2=([string]$b.line2).PadRight($chars).Substring(0,$chars);$sp.Write([char]12);$sp.Write($line1+$line2);$sp.Close();Send-Json $s 200 @{ok=$true};return}
