...
async function route(req,res){
 ...
 try{
  ...
  if(req.method==='POST'&&req.url==='/display-window'){const displayScript=`$ErrorActionPreference='Stop';Add-Type -AssemblyName System.Windows.Forms;$screens=[System.Windows.Forms.Screen]::AllScreens;if($screens.Count -lt 2){throw 'No secondary monitor detected.'};$s=$screens[1];$url='http://127.0.0.1:${PORT}/customer-display';$edge=(Get-Command msedge.exe -ErrorAction SilentlyContinue).Source;if(-not $edge){$candidates=@("$env:ProgramFiles\Microsoft\Edge\Application\msedge.exe","$env:ProgramFiles(x86)\Microsoft\Edge\Application\msedge.exe","$env:LOCALAPPDATA\Microsoft\Edge\Application\msedge.exe");$edge=$candidates|Where-Object{Test-Path $_}|Select-Object -First 1};if(-not $edge){$chrome=(Get-Command chrome.exe -ErrorAction SilentlyContinue).Source;if($chrome){$edge=$chrome}};if(-not $edge){throw 'Microsoft Edge or Google Chrome was not found.'};Start-Process $edge -ArgumentList "--app=$url","--new-window","--window-position=$($s.Bounds.X),$($s.Bounds.Y)","--window-size=$($s.Bounds.Width),$($s.Bounds.Height)"`;await ps(displayScript);return json(res,200,{ok:true,monitor:'secondary'});}
