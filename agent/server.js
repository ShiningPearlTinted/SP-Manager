...
function proxyFastReport(req,res){return new Promise((resolve,reject)=>{const up=http.request({hostname:'127.0.0.1',port:18767,path:'/price-tags/pdf',method:'POST',headers:{'Content-Type':req.headers['content-type']||'application/json','Content-Length':req.headers['content-length']||'0'}},up=>{res.writeHead(up.statusCode||502,{...up.headers,'Access-Control-Allow-Origin':'*','Cache-Control':'no-store'});up.pipe(res);up.on('end',resolve);up.on('error',reject)});up.on('error',reject);req.pipe(up)})}
function ensureFastReport(){return ps(`$p=Get-Process powershell -ErrorAction SilentlyContinue | Where-Object {$_.CommandLine -match 'fastreport-price-tags\.ps1'}; if(!$p){$frPs=Join-Path $env:SystemRoot 'SysWOW64\WindowsPowerShell\v1.0\powershell.exe'; if(!(Test-Path $frPs)){$frPs=Join-Path $env:SystemRoot 'System32\WindowsPowerShell\v1.0\powershell.exe'}; Start-Process -FilePath $frPs -ArgumentList '-NoProfile','-NonInteractive','-ExecutionPolicy','Bypass','-WindowStyle','Hidden','-File',(Join-Path $PSScriptRoot 'fastreport-price-tags.ps1') -WindowStyle Hidden}`)}
async function route(req,res){
 if(req.method==='OPTIONS')return json(res,204,{});
 try{
  if(req.method==='POST'&&req.url==='/price-tags/pdf'){try{await proxyFastReport(req,res);return}catch(e){try{await ensureFastReport();}catch{};await new Promise(r=>setTimeout(r,500));await proxyFastReport(req,res);return}}
